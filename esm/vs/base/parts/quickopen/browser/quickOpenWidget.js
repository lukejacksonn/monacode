...
var QuickOpenController = /** @class */ (function (_super) {
  ...
  QuickOpenController.prototype.onContextMenu = function (
    tree,
    element,
    event
  ) {
...
var DEFAULT_INPUT_ARIA_LABEL = nls.localize(
  'quickOpenAriaLabel',
...
var QuickOpenWidget = /** @class */ (function (_super) {
  ...
  QuickOpenWidget.prototype.create = function () {
    ...
    this._register(
      DOM.addDisposableListener(this.element, DOM.EventType.KEY_DOWN, function (
        e
      ) {
        ...
        if (keyboardEvent.keyCode === 9 /* Escape */) {
          ...
          keyboardEvent.keyCode === 2 /* Tab */ &&
          !keyboardEvent.altKey &&
          !keyboardEvent.ctrlKey &&
          !keyboardEvent.metaKey
        ) {
          var stops = e.currentTarget.querySelectorAll(
          ...
          if (keyboardEvent.shiftKey && keyboardEvent.target === stops[0]) {
            ...
            keyboardEvent.target === stops[stops.length - 1]
          ) {
            DOM.EventHelper.stop(e, true);
            stops[0].focus();
          }
    ...
    this.inputBox = this._register(
      new InputBox(this.inputContainer, undefined, {
        ...
        inputValidationErrorForeground: this.styles
    ...
    this._register(
      DOM.addDisposableListener(
        ...
        function (e) {
          var keyboardEvent = new StandardKeyboardEvent(e);
          var shouldOpenInBackground = _this.shouldOpenInBackground(
            keyboardEvent
          );
          ...
          // Pass tree navigation keys to the tree but leave focus in input field
            ...
            keyboardEvent.keyCode === 18 /* DownArrow */ ||
            keyboardEvent.keyCode === 16 /* UpArrow */ ||
            keyboardEvent.keyCode === 12 /* PageDown */ ||
            keyboardEvent.keyCode === 11 /* PageUp */
          ) {
            DOM.EventHelper.stop(e, true);
            ...
            if (
              _this.inputBox.inputElement.selectionStart ===
              _this.inputBox.inputElement.selectionEnd
            ) {
              _this.inputBox.inputElement.selectionStart =
                _this.inputBox.value.length;
            }
          }
          // Select element on Enter or on Arrow-Right if we are at the end of the input
          else if (
            keyboardEvent.keyCode === 3 /* Enter */ ||
            shouldOpenInBackground
          ) {
            DOM.EventHelper.stop(e, true);
            var focus_1 = _this.tree.getFocus();
            if (focus_1) {
              _this.elementSelected(
                focus_1,
                e,
                shouldOpenInBackground
                  ? 2 /* OPEN_IN_BACKGROUND */
                  : 1 /* OPEN */
              );
            }
          }
    ...
    var createTree =
      this.options.treeCreator ||
      function (container, config, opts) {
    ...
    this._register(
      this.tree.onDidChangeFocus(function (event) {
        _this.elementFocused(event.focus, event);
      })
    );
    this._register(
      this.tree.onDidChangeSelection(function (event) {
        if (event.selection && event.selection.length > 0) {
          var mouseEvent =
            event.payload &&
            event.payload.originalEvent instanceof StandardMouseEvent
              ? event.payload.originalEvent
              : undefined;
          var shouldOpenInBackground = mouseEvent
            ? _this.shouldOpenInBackground(mouseEvent)
            : false;
          _this.elementSelected(
            event.selection[0],
            event,
            shouldOpenInBackground ? 2 /* OPEN_IN_BACKGROUND */ : 1 /* OPEN */
          );
        }
      })
    );
  ...
  QuickOpenWidget.prototype.applyStyles = function () {
    if (this.element) {
      var foreground = this.styles.foreground
      ...
      var borderColor = this.styles.borderColor
        ...
        : '';
      var widgetShadow = this.styles.widgetShadow
      ...
      this.element.style.boxShadow = widgetShadow
        ? '0 5px 8px ' + widgetShadow
        : '';
    }
    if (this.progressBar) {
  ...
  QuickOpenWidget.prototype.shouldOpenInBackground = function (e) {
    ...
    if (e instanceof StandardKeyboardEvent) {
      ...
      return (
        element.selectionEnd === this.inputBox.value.length &&
  ...
  QuickOpenWidget.prototype.elementFocused = function (value, event) {
    ...
    // ARIA
    var arivaActiveDescendant = this.treeElement.getAttribute(
      'aria-activedescendant'
    );
    if (arivaActiveDescendant) {
      this.inputElement.setAttribute(
        'aria-activedescendant',
        arivaActiveDescendant
      );
    } else {
  ...
  QuickOpenWidget.prototype.elementSelected = function (
    value,
    event,
    preferredMode
  ) {
    ...
    return {
      ...
      alt:
        ...
        (event.altKey ||
          (event.payload &&
            event.payload.originalEvent &&
            event.payload.originalEvent.altKey)),
    };
  };
  ...
  QuickOpenWidget.prototype.setInputAndLayout = function (input, autoFocus) {
    ...
    this.tree
      ...
      .then(function () {
        _this.model = input;
        // ARIA
        _this.inputElement.setAttribute(
          'aria-haspopup',
      ...
      .then(function () {
        ...
        _this.tree.layout();
        var entries = input
          ? input.entries.filter(function (e) {
  ...
  QuickOpenWidget.prototype.loosingFocus = function (e) {
    ...
    if (
      ...
      DOM.isAncestor(relatedTarget, this.element)
    ) {
