...
var BaseActionViewItem = /** @class */ (function (_super) {
  ...
  function BaseActionViewItem(context, action, options) {
    ...
    if (action instanceof Action) {
      _this._register(
        action.onDidChange(function (event) {
          if (!_this.element) {
  ...
  BaseActionViewItem.prototype.render = function (container) {
      ...
      if (isFirefox) {
        ...
        this._register(
          DOM.addDisposableListener(
            ...
            function (e) {
              var _a;
              return (_a = e.dataTransfer) === null || _a === void 0
                ? void 0
                : _a.setData(DataTransfers.TEXT, _this._action.label);
            }
          )
        );
      }
    ...
    this._register(
      DOM.addDisposableListener(element, EventType.Tap, function (e) {
        return _this.onClick(e);
      })
    );
    this._register(
      DOM.addDisposableListener(element, DOM.EventType.MOUSE_DOWN, function (
        e
      ) {
        if (!enableDragging) {
    ...
    this._register(
      DOM.addDisposableListener(element, DOM.EventType.CLICK, function (e) {
    ...
    this._register(
      DOM.addDisposableListener(element, DOM.EventType.DBLCLICK, function (e) {
        DOM.EventHelper.stop(e, true);
      })
    );
    [DOM.EventType.MOUSE_UP, DOM.EventType.MOUSE_OUT].forEach(function (event) {
      _this._register(
        DOM.addDisposableListener(element, event, function (e) {
  ...
  function Separator(label) {
    var _this =
      _super.call(
        ...
        Separator.ID,
        label,
        label ? 'separator text' : 'separator'
      ) || this;
    _this.checked = false;
    _this.enabled = false;
    return _this;
...
var ActionBar = /** @class */ (function (_super) {
  ...
  function ActionBar(container, options) {
    ...
    _this._register(
      _this._actionRunner.onDidRun(function (e) {
        return _this._onDidRun.fire(e);
      })
    );
    _this._register(
      _this._actionRunner.onDidBeforeRun(function (e) {
      ...
      DOM.addDisposableListener(
        ...
        function (e) {
          ...
          if (event.equals(previousKey)) {
            ...
            if (
              _this.options.triggerKeys &&
              _this.options.triggerKeys.keyDown
            ) {
    ...
    _this._register(
      DOM.addDisposableListener(_this.domNode, DOM.EventType.KEY_UP, function (
        e
      ) {
        var event = new StandardKeyboardEvent(e);
    ...
    _this._register(
      _this.focusTracker.onDidBlur(function () {
        if (
          document.activeElement === _this.domNode ||
          !DOM.isAncestor(document.activeElement, _this.domNode)
        ) {
    ...
    _this._register(
      _this.focusTracker.onDidFocus(function () {
      ...
      if (i === this.focusedItem) {
        if (types.isFunction(actionViewItem.isEnabled)) {
          if (
            ...
            types.isFunction(actionViewItem.focus)
          ) {
