...
import {
  QuickOpenEntry,
  QuickOpenModel,
} from '../../../../base/parts/quickopen/browser/quickOpenModel.js';
...
var GotoLineEntry = /** @class */ (function (_super) {
  ...
  GotoLineEntry.prototype.parseInput = function (line) {
    ...
    var isValid = model
      ? model.validatePosition(position).equals(position)
      : false;
    var label;
    if (isValid) {
      if (position.column && position.column > 1) {
        label = strings.format(
          GoToLineNLS.gotoLineLabelValidLineAndColumn,
          position.lineNumber,
          position.column
        );
      } else {
        label = strings.format(
          GoToLineNLS.gotoLineLabelValidLine,
          position.lineNumber
        );
      }
    } else if (
      position.lineNumber < 1 ||
      position.lineNumber > (model ? model.getLineCount() : 0)
    ) {
      label = strings.format(
        GoToLineNLS.gotoLineLabelEmptyWithLineLimit,
        model ? model.getLineCount() : 0
      );
    } else {
      label = strings.format(
        GoToLineNLS.gotoLineLabelEmptyWithLineAndColumnLimit,
        model ? model.getLineMaxColumn(position.lineNumber) : 0
      );
    }
    return {
      position: position,
  ...
  GotoLineEntry.prototype.getAriaLabel = function () {
    ...
    return strings.format(
      GoToLineNLS.gotoLineAriaLabel,
      currentLine,
      this.parseResult.label
    );
  };
...
var GotoLineAction = /** @class */ (function (_super) {
  ...
  function GotoLineAction() {
    return (
      _super.call(this, GoToLineNLS.gotoLineActionInput, {
    ...
    this._show(this.getController(editor), {
      getModel: function (value) {
        return new QuickOpenModel([
          new GotoLineEntry(value, editor, _this.getController(editor)),
        ]);
      },
