...
var QuickOutlineAction = /** @class */ (function (_super) {
  ...
  function QuickOutlineAction() {
    return (
      _super.call(this, QuickOutlineNLS.quickOutlineActionInput, {
  ...
  QuickOutlineAction.prototype.run = function (accessor, editor) {
    ...
    // Resolve outline
    return getDocumentSymbols(model, true, CancellationToken.None).then(
      function (result) {
        if (result.length === 0) {
          return;
        }
    ...
    this._show(this.getController(editor), {
      getModel: function (value) {
        return new QuickOpenModel(
          _this.toQuickOpenEntries(editor, result, value)
        );
      },
  ...
  QuickOutlineAction.prototype.symbolEntry = function (
    name,
    type,
    description,
    range,
  ...
  QuickOutlineAction.prototype.toQuickOpenEntries = function (
    editor,
    flattened,
    searchValue
  ) {
    ...
    for (var _i = 0, flattened_1 = flattened; _i < flattened_1.length; _i++) {
      ...
      if (highlights) {
        ...
        results.push(
          this.symbolEntry(
            label,
            SymbolKinds.toCssClassName(element.kind),
            description,
            element.range,
            highlights,
            editor,
    ...
    // Mark first entry as outline
      ...
      results[0].setGroupLabel(
        strings.format(QuickOutlineNLS._symbols_, results.length)
      );
    }
    return results;
  };
  ...
  QuickOutlineAction.prototype.sortNormal = function (
    searchValue,
    elementA,
    elementB
  ) {
  ...
  QuickOutlineAction.prototype.sortScoped = function (
    searchValue,
    elementA,
    elementB
  ) {
