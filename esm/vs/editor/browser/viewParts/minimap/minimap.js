...
} from '../../../../base/browser/globalMouseMoveMonitor.js';
...
var MinimapOptions = /** @class */ (function () {
  function MinimapOptions(configuration) {
    ...
    this.charRenderer = once(function () {
      return MinimapCharRendererFactory.create(
        _this.fontScale,
        fontInfo.fontFamily
      );
    });
...
var MinimapLayout = /** @class */ (function () {
  ...
  MinimapLayout.prototype.getDesiredScrollTopFromTouchLocation = function (
    pageY
  ) {
    return Math.round(
  ...
  MinimapLayout.create = function (
    ...
    var minimapLineHeight = getMinimapLineHeight(
      options.renderMinimap,
      options.fontScale
    );
    var minimapLinesFitting = Math.floor(
    ...
    if (options.scrollBeyondLastLine) {
      ...
      maxMinimapSliderTop = Math.max(
        0,
        (lineCount * minimapLineHeight) / pixelRatio - sliderHeight
      );
    }
...
var MinimapBuffers = /** @class */ (function () {
  ...
  MinimapBuffers._createBackgroundFillData = function (
    WIDTH,
    HEIGHT,
    background
  ) {
...
var Minimap = /** @class */ (function (_super) {
  ...
  function Minimap(context) {
    ...
    _this._decorationsCanvas = createFastDomNode(
    ...
    _this._applyLayout();
    _this._mouseDownListener = dom.addStandardDisposableListener(
      ...
      function (e) {
        ...
        var minimapLineHeight = getMinimapLineHeight(
          renderMinimap,
          _this._options.fontScale
        );
        var internalOffsetY =
          _this._options.pixelRatio * e.browserEvent.offsetY;
        var lineIndex = Math.floor(internalOffsetY / minimapLineHeight);
        var lineNumber =
          lineIndex + _this._lastRenderData.renderedLayout.startLineNumber;
        lineNumber = Math.min(lineNumber, _this._context.model.getLineCount());
        _this._context.privateViewEventBus.emit(
    ...
    _this._sliderMouseDownListener = dom.addStandardDisposableListener(
      _this._slider.domNode,
      'mousedown',
      function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (e.leftButton && _this._lastRenderData) {
          ...
          _this._sliderMouseMoveMonitor.startMonitoring(
            e.target,
            e.buttons,
            standardMouseMoveMerger,
            function (mouseMoveData) {
              var mouseOrthogonalDelta = Math.abs(
                mouseMoveData.posx - initialMouseOrthogonalPosition_1
              );
              if (
                platform.isWindows &&
              ...
              _this._context.viewLayout.setScrollPositionNow({
                scrollTop: initialSliderState_1.getDesiredScrollTopFromDelta(
                  mouseDelta
                ),
              });
            },
    ...
    _this._sliderTouchStartListener = dom.addDisposableListener(
      _this._domNode.domNode,
      EventType.Start,
      function (e) {
        e.preventDefault();
    ...
    _this._sliderTouchMoveListener = dom.addStandardDisposableListener(
      _this._domNode.domNode,
      EventType.Change,
      function (e) {
        e.preventDefault();
    ...
    _this._sliderTouchEndListener = dom.addStandardDisposableListener(
      _this._domNode.domNode,
      EventType.End,
      function (e) {
        e.preventDefault();
  ...
  Minimap.prototype.scrollDueToTouchEvent = function (touch) {
    ...
    var scrollTop = this._lastRenderData.renderedLayout.getDesiredScrollTopFromTouchLocation(
      touch.pageY - startY
    );
  ...
  Minimap.prototype._getBuffer = function () {
    if (!this._buffers) {
      if (
        this._options.canvasInnerWidth > 0 &&
        this._options.canvasInnerHeight > 0
      ) {
        this._buffers = new MinimapBuffers(
          ...
          this._tokensColorTracker.getColor(2 /* DefaultBackground */)
        );
      }
    }
  ...
  Minimap.prototype.render = function (renderingCtx) {
    ...
    var layout = MinimapLayout.create(
      this._options,
    ...
    var scrollLeftChars =
      renderingCtx.scrollLeft / this._options.typicalHalfwidthCharacterWidth;
    var horizontalSliderLeft = Math.min(
      this._options.minimapWidth,
      Math.round(
        (scrollLeftChars *
          getMinimapCharWidth(
            this._options.renderMinimap,
    ...
    this._sliderHorizontal.setWidth(
      this._options.minimapWidth - horizontalSliderLeft
    );
  ...
  Minimap.prototype.renderDecorations = function (layout) {
    if (this._renderDecorations) {
      ...
      var decorations = this._context.model.getDecorationsInViewport(
        new Range(
          layout.startLineNumber,
          1,
          layout.endLineNumber,
          this._context.model.getLineMaxColumn(layout.endLineNumber)
        )
      );
      ...
      var lineHeight = getMinimapLineHeight(
        renderMinimap,
        this._options.fontScale
      );
      var characterWidth = getMinimapCharWidth(
        renderMinimap,
        this._options.fontScale
      );
      ...
      for (var i = 0; i < decorations.length; i++) {
        ...
        var decorationColor = decoration.options.minimap.getColor(
          this._context.theme
        );
        for (
          var line = decoration.range.startLineNumber;
          line <= decoration.range.endLineNumber;
          line++
        ) {
  ...
  Minimap.prototype.renderDecorationOnLine = function (
    canvasContext,
    lineOffsetMap,
    decorationRange,
    decorationColor,
    ...
    var x =
      startLineNumber === lineNumber
        ? lineIndexToXOffset[startColumn - 1]
        : MINIMAP_GUTTER_WIDTH;
    var endColumnForLine =
      endLineNumber > lineNumber
        ? lineIndexToXOffset.length - 1
        : endColumn - 1;
  ...
  Minimap.prototype.renderLineHighlight = function (
    canvasContext,
    decorationColor,
    y,
    height
    ...
    canvasContext.fillStyle =
      (decorationColor && decorationColor.transparent(0.5).toString()) || '';
  ...
  Minimap.prototype.renderDecoration = function (
    canvasContext,
    decorationColor,
    x,
    y,
  ...
  Minimap.prototype.renderLines = function (layout) {
    ...
    var _a = Minimap._renderUntouchedLines(
        imageData,
        startLineNumber,
        endLineNumber,
        minimapLineHeight,
    ...
    var lineInfo = this._context.model.getMinimapLinesRenderingData(
      startLineNumber,
      endLineNumber,
      needed
    );
    var tabSize = lineInfo.tabSize;
    var background = this._tokensColorTracker.getColor(
      2 /* DefaultBackground */
    );
    ...
    var renderedLines = [];
    for (
      var lineIndex = 0, lineCount = endLineNumber - startLineNumber + 1;
      lineIndex < lineCount;
      lineIndex++
    ) {
      if (needed[lineIndex]) {
  ...
  Minimap._renderUntouchedLines = function (
    target,
    startLineNumber,
    endLineNumber,
    minimapLineHeight,
  ...
  Minimap._renderLine = function (
    ...
    var tabsCharDelta = 0;
    for (
      var tokenIndex = 0, tokensLen = tokens.getCount();
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (sliderBackground) {
    ...
    collector.addRule(
      '.monaco-editor .minimap-slider, .monaco-editor .minimap-slider .minimap-slider-horizontal { background: ' +
  ...
  if (sliderHoverBackground) {
    ...
    collector.addRule(
      '.monaco-editor .minimap-slider:hover, .monaco-editor .minimap-slider:hover .minimap-slider-horizontal { background: ' +
        halfSliderHoverBackground +
        '; }'
    );
  }
  ...
  if (sliderActiveBackground) {
    ...
    collector.addRule(
      '.monaco-editor .minimap-slider.active, .monaco-editor .minimap-slider.active .minimap-slider-horizontal { background: ' +
        halfSliderActiveBackground +
        '; }'
    );
  }
  ...
  if (shadow) {
    collector.addRule(
      '.monaco-editor .minimap-shadow-visible { box-shadow: ' +
        shadow +
        ' -6px 0 6px -6px inset; }'
    );
  }
