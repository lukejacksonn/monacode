...
function getHoverMessage(link, useMetaKey) {
  ...
  if (link.url) {
    var hoverMessage = new MarkdownString('', true).appendMarkdown(
      '[' + label + '](' + link.url.toString() + ') (' + kb + ')'
    );
    return hoverMessage;
  } else {
...
var LinkDetector = /** @class */ (function () {
  function LinkDetector(editor, openerService, notificationService) {
    ...
    this.listenersToRemove.add(
      clickLinkGesture.onMouseMoveOrRelevantKeyDown(function (_a) {
        var mouseEvent = _a[0],
          keyboardEvent = _a[1];
    ...
    this.listenersToRemove.add(
      clickLinkGesture.onExecute(function (e) {
        _this.onEditorMouseUp(e);
      })
    );
    this.listenersToRemove.add(
      clickLinkGesture.onCancel(function (e) {
    ...
    this.listenersToRemove.add(
      LinkProviderRegistry.onDidChange(function (e) {
        return _this.onModelModeChanged();
      })
    );
      ...
      return __generator(this, function (_b) {
        switch (_b.label) {
          case 0:
            ...
            this.computePromise = async.createCancelablePromise(function (
              token
            ) {
  ...
  LinkDetector.prototype.updateDecorations = function (links) {
    var useMetaKey =
      this.editor.getOption(59 /* multiCursorModifier */) === 'altKey';
    var oldDecorations = [];
  ...
  LinkDetector.prototype.openLinkOccurrence = function (
    occurrence,
    openToSide,
    fromUserGesture
  ) {
    ...
    link.resolve(CancellationToken.None).then(
      ...
      function (err) {
        ...
        if (messageOrError === 'invalid') {
          _this.notificationService.warn(
            nls.localize(
              'invalid.url',
  ...
  LinkDetector.prototype.getLinkOccurrence = function (position) {
    ...
    for (
      var _i = 0, decorations_1 = decorations;
      _i < decorations_1.length;
      _i++
    ) {
  ...
  LinkDetector = __decorate(
    [__param(1, IOpenerService), __param(2, INotificationService)],
    LinkDetector
  );
  return LinkDetector;
})();
