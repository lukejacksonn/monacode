...
var ModesHoverController = /** @class */ (function () {
  function ModesHoverController(
    ...
    this._hookEvents();
    this._didChangeConfigurationHandler = this._editor.onDidChangeConfiguration(
      function (e) {
  ...
  ModesHoverController.prototype._hookEvents = function () {
    ...
    if (this._isHoverEnabled) {
      ...
      this._toUnhook.add(
        this._editor.onMouseUp(function (e) {
          return _this._onEditorMouseUp(e);
        })
      );
      this._toUnhook.add(
        this._editor.onMouseMove(function (e) {
      ...
      this._toUnhook.add(
        this._editor.onDidChangeModelDecorations(function () {
    ...
    this._toUnhook.add(
      this._editor.onDidScrollChange(function (e) {
  ...
  ModesHoverController.prototype._onEditorMouseMove = function (mouseEvent) {
    ...
    if (
      this._isMouseDown &&
      this._hoverClicked &&
      this.contentWidget.isColorPickerVisible()
    ) {
      return;
    }
    if (
    ...
    if (targetType === 7 /* CONTENT_EMPTY */) {
      var epsilon =
        this._editor.getOption(34 /* fontInfo */)
          .typicalHalfwidthCharacterWidth / 2;
      var data = mouseEvent.target.detail;
      if (
        data &&
        !data.isAfterLines &&
        typeof data.horizontalDistanceToText === 'number' &&
        data.horizontalDistanceToText < epsilon
      ) {
    ...
    if (targetType === 6 /* CONTENT_TEXT */) {
      ...
      if (this._isHoverEnabled && mouseEvent.target.range) {
        this.contentWidget.startShowingAt(
  ...
  ModesHoverController.prototype._createHoverWidgets = function () {
    this._contentWidget.value = new ModesContentHoverWidget(
      this._editor,
      this._markerDecorationsService,
      this._themeService,
      this._keybindingService,
      this._modeService,
      this._openerService
    );
    this._glyphWidget.value = new ModesGlyphHoverWidget(
      this._editor,
      this._modeService,
      this._openerService
    );
  };
  ModesHoverController.prototype.showContentHover = function (
    range,
    mode,
    focus
  ) {
...
var ShowHoverAction = /** @class */ (function (_super) {
  ...
  ShowHoverAction.prototype.run = function (accessor, editor) {
    ...
    var focus =
      editor.getOption(2 /* accessibilitySupport */) === 2; /* Enabled */
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (hoverBorder) {
    ...
    collector.addRule(
      '.monaco-editor .monaco-editor-hover .hover-row:not(:first-child):not(:empty) { border-top: 1px solid ' +
        hoverBorder.transparent(0.5) +
        '; }'
    );
    collector.addRule(
      '.monaco-editor .monaco-editor-hover hr { border-top: 1px solid ' +
        hoverBorder.transparent(0.5) +
        '; }'
    );
    collector.addRule(
      '.monaco-editor .monaco-editor-hover hr { border-bottom: 0px solid ' +
        hoverBorder.transparent(0.5) +
        '; }'
    );
  }
  ...
  if (hoverForeground) {
    collector.addRule(
      '.monaco-editor .monaco-editor-hover { color: ' + hoverForeground + '; }'
    );
  }
  var actionsBackground = theme.getColor(editorHoverStatusBarBackground);
  if (actionsBackground) {
    collector.addRule(
      '.monaco-editor .monaco-editor-hover .hover-row .actions { background-color: ' +
        actionsBackground +
        '; }'
    );
  }
  ...
  if (codeBackground) {
    collector.addRule(
      '.monaco-editor .monaco-editor-hover code { background-color: ' +
        codeBackground +
        '; }'
    );
  }
