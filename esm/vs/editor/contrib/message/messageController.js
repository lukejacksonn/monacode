...
var MessageController = /** @class */ (function (_super) {
  ...
  function MessageController(editor, contextKeyService) {
    ...
    _this._visible = MessageController.MESSAGE_VISIBLE.bindTo(
      contextKeyService
    );
    _this._register(
      _this._editor.onDidAttemptReadOnlyEdit(function () {
  ...
  MessageController.prototype.showMessage = function (message, position) {
    ...
    this._messageListeners.add(
      this._editor.onDidDispose(function () {
        return _this.closeMessage();
      })
    );
    this._messageListeners.add(
  ...
  MessageController.prototype._onDidAttemptReadOnlyEdit = function () {
    if (this._editor.hasModel()) {
      this.showMessage(
        nls.localize('editor.readonly', 'Cannot edit in read-only editor'),
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (border) {
    ...
    collector.addRule(
      '.monaco-editor .monaco-editor-overlaymessage .anchor { border-top-color: ' +
        border +
        '; }'
    );
    collector.addRule(
      '.monaco-editor .monaco-editor-overlaymessage .message { border: ' +
        borderWidth +
        'px solid ' +
        border +
        '; }'
  ...
  if (background) {
    collector.addRule(
      '.monaco-editor .monaco-editor-overlaymessage .message { background-color: ' +
        background +
        '; }'
    );
  }
  ...
  if (foreground) {
    collector.addRule(
      '.monaco-editor .monaco-editor-overlaymessage .message { color: ' +
        foreground +
        '; }'
    );
  }
