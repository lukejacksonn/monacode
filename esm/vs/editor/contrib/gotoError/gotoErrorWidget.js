...
var MessageWidget = /** @class */ (function () {
  function MessageWidget(parent, editor, onRelatedInformation, _openerService) {
    ...
    this._disposables.add(
      dom.addStandardDisposableListener(this._relatedBlock, 'click', function (
        event
      ) {
    ...
    this._disposables.add(
      this._scrollable.onScroll(function (e) {
  ...
  MessageWidget.prototype.update = function (_a) {
    ...
    var source = _a.source,
      ...
      code = _a.code;
    var sourceAndCodeLength =
      ((source === null || source === void 0 ? void 0 : source.length) || 0) +
      '()'.length;
    if (code) {
    ...
    if (isNonEmptyArray(relatedInformation)) {
      var relatedInformationNode = this._relatedBlock.appendChild(
        document.createElement('div')
      );
      relatedInformationNode.style.paddingTop =
        Math.floor(this._editor.getOption(49 /* lineHeight */) * 0.66) + 'px';
      this._lines += 1;
      for (
        var _c = 0, relatedInformation_1 = relatedInformation;
        _c < relatedInformation_1.length;
        _c++
      ) {
        ...
        relatedResource.innerHTML =
          getBaseLabel(related.resource) +
          '(' +
          related.startLineNumber +
          ', ' +
          related.startColumn +
          '): ';
...
var MarkerNavigationWidget = /** @class */ (function (_super) {
  ...
  function MarkerNavigationWidget(
    editor,
    actions,
    _themeService,
    _openerService
    ...
    _this.onDidSelectRelatedInformation =
      _this._onDidSelectRelatedInformation.event;
    ...
    _this._callOnDispose.add(
      _themeService.onThemeChange(_this._applyTheme.bind(_this))
    );
    _this.create();
    return _this;
  }
  ...
  MarkerNavigationWidget.prototype.showAtMarker = function (
    marker,
    markerIdx,
    markerCount
  ) {
    ...
    this._icon.className =
      'codicon ' +
      SeverityIcon.className(MarkerSeverity.toSeverity(this._severity));
  ...
  MarkerNavigationWidget.prototype._doLayoutBody = function (
    heightInPixel,
    widthInPixel
  ) {
...
export var editorMarkerNavigationError = registerColor(
...
export var editorMarkerNavigationWarning = registerColor(
...
export var editorMarkerNavigationInfo = registerColor(
  'editorMarkerNavigationInfo.background',
  { dark: infoDefault, light: infoDefault, hc: infoDefault },
  nls.localize(
    'editorMarkerNavigationInfo',
    'Editor marker navigation widget info color.'
  )
);
export var editorMarkerNavigationBackground = registerColor(
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (linkFg) {
    collector.addRule(
      '.monaco-editor .marker-widget a { color: ' + linkFg + '; }'
    );
    collector.addRule(
      '.monaco-editor .marker-widget a.code-link span:hover { color: ' +
        linkFg +
        '; }'
    );
  }
