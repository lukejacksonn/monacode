...
var DecorationsManager = /** @class */ (function () {
  ...
  DecorationsManager.prototype._addDecorations = function (reference) {
    ...
    this._callOnModelChange.add(
      this._editor.getModel().onDidChangeDecorations(function (event) {
...
var ReferenceWidget = /** @class */ (function (_super) {
  ...
  function ReferenceWidget(
    ...
    _this._callOnDispose.add(
      themeService.onThemeChange(_this._applyTheme.bind(_this))
    );
  ...
  ReferenceWidget.prototype._fillBody = function (containerElement) {
    ...
    this._previewNotAvailableMessage = TextModel.createFromString(
    ...
    this._disposables.add(
      this._splitView.onDidSashChange(function () {
        if (_this._dim.width) {
          _this.layoutData.ratio =
            _this._splitView.getViewSize(0) / _this._dim.width;
        }
      }, undefined)
    );
    ...
    this._tree.onDidOpen(function (e) {
      if (
        ...
        e.browserEvent instanceof KeyboardEvent ||
        (e.browserEvent instanceof MouseEvent && e.browserEvent.detail === 2) ||
        e.browserEvent.tapCount === 2
      ) {
  ...
  ReferenceWidget.prototype._doLayoutBody = function (
    heightInPixel,
    widthInPixel
  ) {
  ...
  ReferenceWidget.prototype._onNewModel = function () {
    ...
    if (this._model.isEmpty) {
      this.setTitle('');
      this._messageContainer.innerHTML = nls.localize(
        'noResults',
        'No results'
      );
    ...
    this._disposeOnNewModel.add(
      this._model.onDidChangeReferenceRange(function (reference) {
        return _this._tree.rerender(reference);
      })
    );
    // listen on editor
    this._disposeOnNewModel.add(
      this._preview.onMouseDown(function (e) {
        var event = e.event,
          target = e.target;
        if (event.detail !== 2) {
          return;
  ...
  ReferenceWidget.prototype._revealReference = function (
    reference,
    revealParent
  ) {
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            ...
            promise = this._textModelResolverService.createModelReference(
              reference.uri
            );
...
registerThemingParticipant(function (theme, collector) {
  var findMatchHighlightColor = theme.getColor(
    peekView.peekViewResultsMatchHighlight
  );
  if (findMatchHighlightColor) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .ref-tree .referenceMatch .highlight { background-color: ' +
        findMatchHighlightColor +
        '; }'
    );
  }
  var referenceHighlightColor = theme.getColor(
    peekView.peekViewEditorMatchHighlight
  );
  if (referenceHighlightColor) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .preview .reference-decoration { background-color: ' +
        referenceHighlightColor +
        '; }'
    );
  }
  var referenceHighlightBorder = theme.getColor(
    peekView.peekViewEditorMatchHighlightBorder
  );
  if (referenceHighlightBorder) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .preview .reference-decoration { border: 2px solid ' +
        referenceHighlightBorder +
  ...
  if (hcOutline) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .ref-tree .referenceMatch .highlight { border: 1px dotted ' +
        hcOutline +
        '; box-sizing: border-box; }'
    );
  }
  ...
  if (resultsBackground) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .ref-tree { background-color: ' +
        resultsBackground +
        '; }'
    );
  }
  var resultsMatchForeground = theme.getColor(
    peekView.peekViewResultsMatchForeground
  );
  if (resultsMatchForeground) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .ref-tree { color: ' +
        resultsMatchForeground +
        '; }'
    );
  }
  var resultsFileForeground = theme.getColor(
    peekView.peekViewResultsFileForeground
  );
  if (resultsFileForeground) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .ref-tree .reference-file { color: ' +
        resultsFileForeground +
        '; }'
    );
  }
  var resultsSelectedBackground = theme.getColor(
    peekView.peekViewResultsSelectionBackground
  );
  if (resultsSelectedBackground) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .ref-tree .monaco-list:focus .monaco-list-rows > .monaco-list-row.selected:not(.highlighted) { background-color: ' +
        resultsSelectedBackground +
        '; }'
    );
  }
  var resultsSelectedForeground = theme.getColor(
    peekView.peekViewResultsSelectionForeground
  );
  if (resultsSelectedForeground) {
    collector.addRule(
      '.monaco-editor .reference-zone-widget .ref-tree .monaco-list:focus .monaco-list-rows > .monaco-list-row.selected:not(.highlighted) { color: ' +
        resultsSelectedForeground +
        ' !important; }'
    );
  }
  ...
  var editorGutterBackground = theme.getColor(
    peekView.peekViewEditorGutterBackground
  );
