...
var TextAreaHandler = /** @class */ (function (_super) {
  ...
  function TextAreaHandler(context, viewController, viewHelper) {
    ...
    _this._emptySelectionClipboard = options.get(
      25 /* emptySelectionClipboard */
    );
    _this._copyWithSyntaxHighlighting = options.get(
      15 /* copyWithSyntaxHighlighting */
    );
    ...
    var textAreaInputHost = {
      getDataToCopy: function (generateHTML) {
        var rawTextToCopy = _this._context.model.getPlainTextToCopy(
          _this._modelSelections,
          _this._emptySelectionClipboard,
          platform.isWindows
        );
        var newLineCharacter = _this._context.model.getEOL();
        var isFromEmptySelection =
          _this._emptySelectionClipboard &&
          _this._modelSelections.length === 1 &&
          _this._modelSelections[0].isEmpty();
        var multicursorText = Array.isArray(rawTextToCopy)
          ? rawTextToCopy
          : null;
        var text = Array.isArray(rawTextToCopy)
          ? rawTextToCopy.join(newLineCharacter)
          : rawTextToCopy;
        var html = undefined;
        var mode = null;
        if (generateHTML) {
          if (
            ...
            (_this._copyWithSyntaxHighlighting && text.length < 65536)
          ) {
            var richText = _this._context.model.getRichTextToCopy(
              _this._modelSelections,
              _this._emptySelectionClipboard
            );
            if (richText) {
      ...
      getScreenReaderContent: function (currentState) {
        ...
        return PagedScreenReaderStrategy.fromEditorSelection(
          ...
          simpleModel,
          _this._selections[0],
          _this._accessibilityPageSize,
          _this._accessibilitySupport === 0 /* Unknown */
        );
      },
      deduceModelPosition: function (
        viewAnchorPosition,
        deltaOffset,
        lineFeedCnt
      ) {
        return _this._context.model.deduceModelPositionRelativeToViewPosition(
          viewAnchorPosition,
          deltaOffset,
          lineFeedCnt
        );
    ...
    _this._textAreaInput = _this._register(
      new TextAreaInput(textAreaInputHost, _this.textArea)
    );
    _this._register(
      _this._textAreaInput.onKeyDown(function (e) {
        _this._viewController.emitKeyDown(e);
      })
    );
    _this._register(
      _this._textAreaInput.onKeyUp(function (e) {
        _this._viewController.emitKeyUp(e);
      })
    );
    _this._register(
      _this._textAreaInput.onPaste(function (e) {
        var pasteOnNewLine = false;
        var multicursorText = null;
        var mode = null;
        if (e.metadata) {
          pasteOnNewLine =
            _this._emptySelectionClipboard && !!e.metadata.isFromEmptySelection;
          multicursorText =
    ...
    _this._register(
      _this._textAreaInput.onCut(function () {
        _this._viewController.cut('keyboard');
      })
    );
    _this._register(
      _this._textAreaInput.onType(function (e) {
        if (e.replaceCharCnt) {
          _this._viewController.replacePreviousChar(
            'keyboard',
            e.text,
            e.replaceCharCnt
          );
    ...
    _this._register(
      _this._textAreaInput.onSelectionChangeRequest(function (modelSelection) {
        _this._viewController.setSelection('keyboard', modelSelection);
      })
    );
    _this._register(
      _this._textAreaInput.onCompositionStart(function () {
        ...
        var visibleRange = _this._viewHelper.visibleRangeForPositionRelativeToEditor(
          lineNumber,
          column
        );
        if (visibleRange) {
          _this._visibleTextArea = new VisibleTextAreaData(
            _this._context.viewLayout.getVerticalOffsetForLineNumber(
              lineNumber
            ),
            visibleRange.left,
    ...
    _this._register(
      _this._textAreaInput.onCompositionUpdate(function (e) {
        if (browser.isEdgeOrIE) {
          ...
          _this._visibleTextArea = _this._visibleTextArea.setWidth(
            measureText(e.data, _this._fontInfo)
          );
        }
        _this._render();
      })
    ...
    _this._register(
      _this._textAreaInput.onCompositionEnd(function () {
        _this._visibleTextArea = null;
        _this._render();
        _this.textArea.setClassName('inputarea');
    ...
    _this._register(
      _this._textAreaInput.onFocus(function () {
        _this._context.privateViewEventBus.emit(
    ...
    _this._register(
      _this._textAreaInput.onBlur(function () {
        _this._context.privateViewEventBus.emit(
  ...
  TextAreaHandler.prototype._getWordBeforePosition = function (position) {
    ...
    var wordSeparators = getMapForWordSeparators(
      this._context.configuration.options.get(96 /* wordSeparators */)
    );
  ...
  TextAreaHandler.prototype._setAccessibilityOptions = function (options) {
    ...
    if (
      this._accessibilitySupport === 2 /* Enabled */ &&
  ...
  TextAreaHandler.prototype._renderInsideEditor = function (
    renderedPosition,
    top,
    left,
    width,
