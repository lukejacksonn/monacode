...
var DiffReview = /** @class */ (function (_super) {
  ...
  function DiffReview(diffEditor) {
    ...
    _this._actionBar = _this._register(
      new ActionBar(_this.actionBarContainer.domNode)
    );
    _this._actionBar.push(
      new Action(
        'diffreview.close',
    ...
    _this.scrollbar = _this._register(
      new DomScrollableElement(_this._content.domNode, {})
    );
    ...
    _this._register(
      diffEditor.getModifiedEditor().onDidChangeCursorPosition(function () {
        if (!_this._isVisible) {
          return;
        }
        _this._render();
    ...
    _this._register(
      diffEditor.getOriginalEditor().onDidFocusEditorWidget(function () {
        if (_this._isVisible) {
          _this.hide();
        }
      })
    ...
    _this._register(
      diffEditor.getModifiedEditor().onDidFocusEditorWidget(function () {
        if (_this._isVisible) {
          _this.hide();
        }
      })
  ...
  DiffReview.prototype.prev = function () {
    ...
    this._diffEditor.setPosition(
      new Position(this._diffs[index].entries[0].modifiedLineStart, 1)
    );
  ...
  DiffReview.prototype._compute = function () {
    ...
    return DiffReview._mergeAdjacent(
      lineChanges,
      originalModel.getLineCount(),
      modifiedModel.getLineCount()
    );
  };
  DiffReview._mergeAdjacent = function (
    lineChanges,
    originalLineCount,
    modifiedLineCount
  ) {
    ...
    for (var i = 0, len = lineChanges.length; i < len; i++) {
      ...
      // Emit before anchors
      {
        var originalEqualAbove =
          originalEnd === 0 ? originalStart : originalStart - 1;
        var modifiedEqualAbove =
          modifiedEnd === 0 ? modifiedStart : modifiedStart - 1;
      ...
      // Emit after anchors
      {
        var originalEqualBelow =
          originalEnd === 0 ? originalStart + 1 : originalEnd + 1;
        var modifiedEqualBelow =
          modifiedEnd === 0 ? modifiedStart + 1 : modifiedEnd + 1;
    ...
    for (var i = 1, len = diffs.length; i < len; i++) {
      ...
      if (
        currLast.getType() === 0 /* Equal */ &&
        thisFirst.getType() === 0 /* Equal */ &&
        thisFirst.originalLineStart <= currLast.originalLineEnd
      ) {
        // We are dealing with equal lines that overlap
        curr[curr.length - 1] = new DiffEntry(
          currLast.originalLineStart,
          thisFirst.originalLineEnd,
          currLast.modifiedLineStart,
          thisFirst.modifiedLineEnd
        );
  ...
  DiffReview.prototype._render = function () {
    ...
    Configuration.applyFontInfoSlow(
      container,
      modifiedOptions.get(34 /* fontInfo */)
    );
    ...
    for (var i = 0, len = diffs.length; i < len; i++) {
      ...
      if (
        originalLineStart !== 0 &&
        (minOriginalLine === 0 || originalLineStart < minOriginalLine)
      ) {
        minOriginalLine = originalLineStart;
      }
      if (
        originalLineEnd !== 0 &&
        (maxOriginalLine === 0 || originalLineEnd > maxOriginalLine)
      ) {
        maxOriginalLine = originalLineEnd;
      }
      if (
        modifiedLineStart !== 0 &&
        (minModifiedLine === 0 || modifiedLineStart < minModifiedLine)
      ) {
        minModifiedLine = modifiedLineStart;
      }
      if (
        modifiedLineEnd !== 0 &&
        (maxModifiedLine === 0 || modifiedLineEnd > maxModifiedLine)
      ) {
    ...
    cell.appendChild(
      document.createTextNode(
        diffIndex +
          ...
          minOriginalLine +
          ',' +
          originalChangedLinesCnt +
          ' +' +
          minModifiedLine +
          ',' +
          modifiedChangedLinesCnt +
          ' @@'
      )
    );
  ...
  DiffReview._renderSection = function (
    dest,
    diffEntry,
    modLine,
    width,
    ...
    var originalLineNumbersWidth =
      originalLayoutInfo.glyphMarginWidth + originalLayoutInfo.lineNumbersWidth;
    var modifiedLayoutInfo = modifiedOptions.get(107 /* layoutInfo */);
    var modifiedLineNumbersWidth =
      10 +
    ...
    for (var i = 0; i <= cnt; i++) {
      ...
      if (originalLine !== 0) {
        originalLineNumber.appendChild(
          document.createTextNode(String(originalLine))
        );
      } else {
      ...
      if (modifiedLine !== 0) {
        modifiedLineNumber.appendChild(
          document.createTextNode(String(modifiedLine))
        );
      } else {
  ...
  DiffReview._renderLine = function (model, options, tabSize, lineNumber) {
    ...
    var defaultMetadata =
      ((0 /* None */ << 11) /* FONT_STYLE_OFFSET */ |
        ...
        (2 /* DefaultBackground */ << 23) /* BACKGROUND_OFFSET */) >>>
      0;
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (lineNumbers) {
    collector.addRule(
      '.monaco-diff-editor .diff-review-line-number { color: ' +
        lineNumbers +
        '; }'
    );
  }
  ...
  if (shadow) {
    collector.addRule(
      '.monaco-diff-editor .diff-review-shadow { box-shadow: ' +
        shadow +
        ' 0 -6px 6px -6px inset; }'
    );
  }
