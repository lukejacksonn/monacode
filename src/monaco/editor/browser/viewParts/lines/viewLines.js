...
import {
  LineVisibleRanges,
  HorizontalPosition,
} from '../../../common/view/renderingContext.js';
var LastRenderedData = /** @class */ (function () {
  ...
  LastRenderedData.prototype.setCurrentVisibleRange = function (
    currentVisibleRange
  ) {
...
var HorizontalRevealRequest = /** @class */ (function () {
  function HorizontalRevealRequest(
    lineNumber,
    startColumn,
    endColumn,
    startScrollTop,
...
var ViewLines = /** @class */ (function (_super) {
  ...
  function ViewLines(context, linesContent) {
    ...
    _this._revealHorizontalRightPadding = options.get(
      75 /* revealHorizontalRightPadding */
    );
    _this._cursorSurroundingLines = options.get(
      19 /* cursorSurroundingLines */
    );
    _this._cursorSurroundingLinesStyle = options.get(
      20 /* cursorSurroundingLinesStyle */
    );
    _this._canUseLayerHinting = !options.get(22 /* disableLayerHinting */);
    _this._viewLineOptions = new ViewLineOptions(
      conf,
      _this._context.theme.type
    );
  ...
  ViewLines.prototype._onOptionsMaybeChanged = function () {
    ...
    var newViewLineOptions = new ViewLineOptions(
      conf,
  ...
  ViewLines.prototype.onCursorStateChanged = function (e) {
    ...
    var r = false;
    for (
      var lineNumber = rendStartLineNumber;
  ...
  ViewLines.prototype.onRevealRangeRequest = function (e) {
    ...
    var desiredScrollTop = this._computeScrollTopToRevealRange(
    ...
    if (e.revealHorizontal) {
      if (e.range.startLineNumber !== e.range.endLineNumber) {
        ...
        this._horizontalRevealRequest = new HorizontalRevealRequest(
  ...
  ViewLines.prototype.linesVisibleRangesForRange = function (
    _range,
    includeNewLines
  ) {
    ...
    var range = Range.intersectRanges(
      _range,
    ...
    var visibleRanges = [],
      visibleRangesLen = 0;
    var domReadingContext = new DomReadingContext(
    ...
    for (
      ...
      var visibleRangesForLine = this._visibleLines
        .getVisibleLine(lineNumber)
  ...
  ViewLines.prototype._visibleRangesForLineRange = function (
    lineNumber,
    startColumn,
    endColumn
  ) {
    ...
    return this._visibleLines
      ...
      .getVisibleRangesForRange(
        startColumn,
        endColumn,
        new DomReadingContext(this.domNode.domNode, this._textRangeRestingSpot)
      );
  };
  ViewLines.prototype.visibleRangeForPosition = function (position) {
    var visibleRanges = this._visibleRangesForLineRange(
      position.lineNumber,
      position.column,
      position.column
    );
  ...
  ViewLines.prototype.renderText = function (viewportData) {
    ...
    if (this._horizontalRevealRequest) {
      ...
      if (
        viewportData.startLineNumber <= revealLineNumber &&
        revealLineNumber <= viewportData.endLineNumber
      ) {
        this._horizontalRevealRequest = null;
        // allow `visibleRangesForRange2` to work
        this.onDidRender();
        // compute new scroll position
        var newScrollLeft = this._computeScrollLeftToRevealRange(
          revealLineNumber,
          revealStartColumn,
          revealEndColumn
        );
  ...
  ViewLines.prototype._computeScrollTopToRevealRange = function (
    viewport,
    source,
    range,
    verticalType
    ...
    var shouldIgnoreScrollOff =
      source === 'mouse' && this._cursorSurroundingLinesStyle === 'default';
    ...
    if (boxEndY - boxStartY > viewportHeight) {
      ...
      verticalType === 2 /* CenterIfOutsideViewport */
    ) {
      if (
        verticalType === 2 /* CenterIfOutsideViewport */ &&
      ...
      newScrollTop = this._computeMinimumScrolling(
        ...
        boxEndY,
        verticalType === 3 /* Top */,
        verticalType === 4 /* Bottom */
      );
    }
    return newScrollTop;
  };
  ViewLines.prototype._computeScrollLeftToRevealRange = function (
    lineNumber,
    startColumn,
    endColumn
  ) {
    ...
    var newScrollLeft = this._computeMinimumScrolling(
      viewportStartX,
      viewportEndX,
      boxStartX,
      boxEndX
  ...
  ViewLines.prototype._computeMinimumScrolling = function (
    viewportStart,
    viewportEnd,
    boxStart,
    boxEnd,
