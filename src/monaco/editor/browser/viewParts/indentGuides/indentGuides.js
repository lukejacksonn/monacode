...
var IndentGuidesOverlay = /** @class */ (function (_super) {
  ...
  function IndentGuidesOverlay(context) {
    ...
    _this._activeIndentEnabled = options.get(
      43 /* highlightActiveIndentGuide */
    );
    _this._maxIndentLeft =
  ...
  IndentGuidesOverlay.prototype.onConfigurationChanged = function (e) {
    ...
    this._activeIndentEnabled = options.get(
      43 /* highlightActiveIndentGuide */
    );
    this._maxIndentLeft =
      wrappingInfo.wrappingColumn === -1
  ...
  IndentGuidesOverlay.prototype.onCursorStateChanged = function (e) {
    ...
    var newPrimaryLineNumber = selection.isEmpty()
  ...
  IndentGuidesOverlay.prototype.prepareRender = function (ctx) {
    ...
    var indents = this._context.model.getLinesIndentGuides(
    ...
    if (this._activeIndentEnabled && this._primaryLineNumber) {
      var activeIndentInfo = this._context.model.getActiveIndentGuide(
        this._primaryLineNumber,
        visibleStartLineNumber,
        visibleEndLineNumber
      );
    ...
    var output = [];
    for (
      var lineNumber = visibleStartLineNumber;
      lineNumber <= visibleEndLineNumber;
      lineNumber++
    ) {
      var containsActiveIndentGuide =
      ...
      var result = '';
      if (indent >= 1) {
        var leftMostVisiblePosition = ctx.visibleRangeForPosition(
  ...
  IndentGuidesOverlay.prototype.render = function (
    startLineNumber,
    lineNumber
  ) {
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (editorIndentGuidesColor) {
    collector.addRule(
      '.monaco-editor .lines-content .cigr { box-shadow: 1px 0 0 0 ' +
        editorIndentGuidesColor +
        ' inset; }'
    );
  }
  var editorActiveIndentGuidesColor =
    theme.getColor(editorActiveIndentGuides) || editorIndentGuidesColor;
  if (editorActiveIndentGuidesColor) {
    collector.addRule(
      '.monaco-editor .lines-content .cigra { box-shadow: 1px 0 0 0 ' +
        editorActiveIndentGuidesColor +
        ' inset; }'
    );
  }
});
