...
var LineNumbersOverlay = /** @class */ (function (_super) {
  ...
  LineNumbersOverlay.prototype.onCursorStateChanged = function (e) {
    ...
    if (
      this._renderLineNumbers === 2 /* Relative */ ||
      this._renderLineNumbers === 3 /* Interval */
    ) {
      return true;
    }
    return false;
  ...
  // --- end event handlers
  LineNumbersOverlay.prototype._getLineRenderLineNumber = function (
    viewLineNumber
  ) {
    var modelPosition = this._context.model.coordinatesConverter.convertViewPositionToModelPosition(
  ...
  LineNumbersOverlay.prototype.prepareRender = function (ctx) {
    ...
    var lineHeightClassName = platform.isLinux
    ...
    for (
      ...
      if (!this._renderFinalNewline) {
        if (
          ...
          this._context.model.getLineLength(lineNumber) === 0
        ) {
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (activeLineNumber) {
    collector.addRule(
      '.monaco-editor .current-line ~ .line-numbers { color: ' +
        activeLineNumber +
        '; }'
    );
  }
