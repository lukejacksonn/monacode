...
var InspectTokensController = /** @class */ (function (_super) {
  ...
  function InspectTokensController(
    editor,
    ...
    _this._widget = null;
    _this._register(
      _this._editor.onDidChangeModel(function (e) {
        return _this.stop();
      })
    );
    _this._register(
      _this._editor.onDidChangeModelLanguage(function (e) {
        return _this.stop();
      })
    );
    _this._register(
  ...
  InspectTokensController = __decorate(
    [__param(1, IStandaloneThemeService), __param(2, IModeService)],
...
function renderTokenText(tokenText) {
  var result = '';
  for (
    var charIndex = 0, len = tokenText.length;
    charIndex < len;
    charIndex++
  ) {
...
function getSafeTokenizationSupport(languageIdentifier) {
  var tokenizationSupport = TokenizationRegistry.get(
...
var InspectTokensWidget = /** @class */ (function (_super) {
  ...
  function InspectTokensWidget(editor, modeService) {
    ...
    _this._tokenizationSupport = getSafeTokenizationSupport(
      _this._model.getLanguageIdentifier()
    );
    _this._compute(_this._editor.getPosition());
    _this._register(
      _this._editor.onDidChangeCursorPosition(function (e) {
  ...
  InspectTokensWidget.prototype._compute = function (position) {
    ...
    result +=
      '<tr><td class="tm-metadata-key">language</td><td class="tm-metadata-value">' +
      escape(metadata.languageIdentifier.language) +
      '</td>';
    result +=
      '<tr><td class="tm-metadata-key">token type</td><td class="tm-metadata-value">' +
      this._tokenTypeToString(metadata.tokenType) +
      '</td>';
    result +=
      '<tr><td class="tm-metadata-key">font style</td><td class="tm-metadata-value">' +
      this._fontStyleToString(metadata.fontStyle) +
      '</td>';
    result +=
      '<tr><td class="tm-metadata-key">foreground</td><td class="tm-metadata-value">' +
      Color.Format.CSS.formatHex(metadata.foreground) +
      '</td>';
    result +=
    ...
    if (token1Index < data.tokens1.length) {
      result +=
        '<span class="tm-token-type">' +
        escape(data.tokens1[token1Index].type) +
        '</span>';
    }
  ...
  InspectTokensWidget.prototype._getTokensAtLine = function (lineNumber) {
    ...
    var tokenizationResult1 = this._tokenizationSupport.tokenize(
      this._model.getLineContent(lineNumber),
      stateBeforeLine,
      0
    );
    var tokenizationResult2 = this._tokenizationSupport.tokenize2(
      this._model.getLineContent(lineNumber),
      stateBeforeLine,
      0
    );
  ...
  InspectTokensWidget.prototype._getStateBeforeLine = function (lineNumber) {
    ...
    for (var i = 1; i < lineNumber; i++) {
      var tokenizationResult = this._tokenizationSupport.tokenize(
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (border) {
    ...
    collector.addRule(
      '.monaco-editor .tokens-inspect-widget { border: ' +
        borderWidth +
        'px solid ' +
        border +
        '; }'
    ...
    collector.addRule(
      '.monaco-editor .tokens-inspect-widget .tokens-inspect-separator { background-color: ' +
        border +
        '; }'
    );
  }
  ...
  if (background) {
    collector.addRule(
      '.monaco-editor .tokens-inspect-widget { background-color: ' +
        background +
        '; }'
    );
  }
  ...
  if (foreground) {
    collector.addRule(
      '.monaco-editor .tokens-inspect-widget { color: ' + foreground + '; }'
    );
  }
});
