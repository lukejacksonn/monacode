...
var CONTEXT_ACCESSIBILITY_WIDGET_VISIBLE = new RawContextKey(
...
function getSelectionLabel(selections, charactersSelected) {
  ...
  if (selections.length === 1) {
    if (charactersSelected) {
      return strings.format(
        AccessibilityHelpNLS.singleSelectionRange,
        selections[0].positionLineNumber,
        selections[0].positionColumn,
        charactersSelected
      );
    }
    return strings.format(
      AccessibilityHelpNLS.singleSelection,
      selections[0].positionLineNumber,
      selections[0].positionColumn
    );
  }
  if (charactersSelected) {
    return strings.format(
      AccessibilityHelpNLS.multiSelectionRange,
      selections.length,
      charactersSelected
    );
  }
  if (selections.length > 0) {
    return strings.format(
      AccessibilityHelpNLS.multiSelection,
      selections.length
    );
  }
  return '';
...
var AccessibilityHelpWidget = /** @class */ (function (_super) {
  ...
  function AccessibilityHelpWidget(
    editor,
    _contextKeyService,
    _keybindingService,
    _openerService
    ...
    _this._editor = editor;
    _this._isVisibleKey = CONTEXT_ACCESSIBILITY_WIDGET_VISIBLE.bindTo(
  ...
  AccessibilityHelpWidget.prototype._descriptionForCommand = function (
    commandId,
    msg,
    noKbMsg
  ) {
  ...
  AccessibilityHelpWidget.prototype._layout = function () {
    ...
    var w = Math.max(
      5,
      Math.min(AccessibilityHelpWidget.WIDTH, editorLayout.width - 40)
    );
    var h = Math.max(
      5,
      Math.min(AccessibilityHelpWidget.HEIGHT, editorLayout.height - 40)
    );
...
var AccessibilityHelpCommand = EditorCommand.bindToContribution(
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (widgetForeground) {
    collector.addRule(
      '.monaco-editor .accessibilityHelpWidget { color: ' +
        widgetForeground +
        '; }'
    );
  }
