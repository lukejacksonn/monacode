 ...
 */
export var editorSuggestWidgetBackground = registerColor(
...
export var editorSuggestWidgetBorder = registerColor(
...
export var editorSuggestWidgetForeground = registerColor(
...
export var editorSuggestWidgetSelectedBackground = registerColor(
...
export var editorSuggestWidgetHighlightForeground = registerColor(
...
function extractColor(item, out) {
  var label =
    typeof item.completion.label === 'string'
  ...
  if (
    ...
    item.completion.documentation.match(colorRegExp)
  ) {
...
var ItemRenderer = /** @class */ (function () {
  ...
  ItemRenderer.prototype.renderTemplate = function (container) {
    ...
    var configureFont = function () {
      ...
      var lineHeight =
        options.get(91 /* suggestLineHeight */) || fontInfo.lineHeight;
    ...
    data.disposables.add(
      Event.chain(this.editor.onDidChangeConfiguration.bind(this.editor))
        .filter(function (e) {
          return (
            e.hasChanged(34 /* fontInfo */) ||
            e.hasChanged(90 /* suggestFontSize */) ||
            e.hasChanged(91 /* suggestLineHeight */)
          );
        })
  ...
  ItemRenderer.prototype.renderElement = function (
    element,
    index,
    templateData
  ) {
    ...
    if (suggestion.kind === 19 /* Color */ && extractColor(element, color)) {
      ...
      suggestion.kind === 20 /* File */ &&
      this._themeService.getIconTheme().hasFileIcons
    ) {
      // special logic for 'file' completion items
      data.icon.className = 'icon hide';
      data.iconContainer.className = 'icon hide';
      var labelClasses = getIconClasses(
        this._modelService,
        this._modeService,
        URI.from({ scheme: 'fake', path: textLabel }),
        FileKind.FILE
      );
      var detailClasses = getIconClasses(
        this._modelService,
        this._modeService,
        URI.from({ scheme: 'fake', path: suggestion.detail }),
        FileKind.FILE
      );
      ...
      addClasses(
        data.iconContainer,
        'suggest-icon codicon codicon-symbol-' +
          completionKindToCssClass(suggestion.kind)
      );
    }
    ...
    if (typeof suggestion.label === 'string') {
      ...
      data.signatureLabel.textContent = (
        suggestion.label.signature || ''
      ).replace(/\n.*$/m, '');
      data.qualifierLabel.textContent = (
        suggestion.label.qualifier || ''
      ).replace(/\n.*$/m, '');
  ...
  SuggestionDetails.prototype.renderItem = function (item, explainMode) {
    ...
    if (explainMode) {
      ...
      md +=
        ...
        item.idx +
        ', based on ' +
        ((item.completion.sortText &&
          'sortText: "' + item.completion.sortText + '"') ||
          'label') +
        '\n';
...
var SuggestWidget = /** @class */ (function () {
  function SuggestWidget(
    ...
    var markdownRenderer = this.toDispose.add(
      new MarkdownRenderer(editor, modeService, openerService)
    );
    var kbToggleDetails =
      (_b =
        (_a = keybindingService.lookupKeybinding('toggleSuggestionDetails')) ===
          null || _a === void 0
          ? void 0
          : _a.getLabel()) !== null && _b !== void 0
        ? _b
        : '';
    this.msgDetailsLess = nls.localize(
      'detail.less',
      '{0} for less...',
      kbToggleDetails
    );
    this.msgDetailMore = nls.localize(
      'detail.more',
      '{0} for more...',
      kbToggleDetails
    );
    ...
    this.toDispose.add(
      addDisposableListener(this.element, 'click', function (e) {
    ...
    var applyStatusBarStyle = function () {
      return toggleClass(
        _this.element,
        'with-status-bar',
        !_this.editor.getOption(89 /* suggest */).hideStatusBar
      );
    };
    applyStatusBarStyle();
    ...
    this.details = instantiationService.createInstance(
      SuggestionDetails,
      this.element,
      this,
      this.editor,
    ...
    var applyIconStyle = function () {
      return toggleClass(
        _this.element,
        'no-icons',
        !_this.editor.getOption(89 /* suggest */).showIcons
      );
    };
    applyIconStyle();
    var renderer = instantiationService.createInstance(
      ItemRenderer,
      this,
      this.editor,
      kbToggleDetails
    ...
    this.toDispose.add(
      addStandardDisposableListener(this.details.element, 'keydown', function (
        e
      ) {
  ...
  SuggestWidget.prototype.onEditorLayoutChange = function () {
    if (
      (this.state === 3 /* Open */ || this.state === 5) /* Details */ &&
  ...
  SuggestWidget.prototype.onListFocus = function (e) {
    ...
    if (item !== this.focusedItem) {
      ...
      var wantsInsert =
        this.editor.getOption(89 /* suggest */).insertMode === 'insert';
      var kbAccept =
        (_a = this.keybindingService.lookupKeybinding(
          'acceptSelectedSuggestion'
        )) === null || _a === void 0
          ? void 0
          : _a.getLabel();
      var kbAcceptAlt =
        (_b = this.keybindingService.lookupKeybinding(
      ...
      if (!Position.equals(item.editInsertEnd, item.editReplaceEnd)) {
        ...
        this.setStatusBarLeftText(
          nls.localize('accept', '{0} to accept', kbAccept)
        );
      }
      if (this.currentSuggestionDetails) {
        this.currentSuggestionDetails.cancel();
      ...
      this.currentSuggestionDetails
        .then(function () {
          if (
            index >= _this.list.length ||
            item !== _this.list.element(index)
          ) {
            return;
          }
  ...
  SuggestWidget.prototype.showSuggestions = function (
    completionModel,
    selectionIndex,
    isFrozen,
    isAuto
  ...
  SuggestWidget.prototype.showDetails = function (loading) {
    ...
    if (loading) {
      ...
      this.details.renderItem(
        this.list.getFocusedElements()[0],
        this.explainMode
      );
    }
  ...
  SuggestWidget.prototype.updateListHeight = function () {
    ...
    if (this.state === 2 /* Empty */ || this.state === 1 /* Loading */) {
      ...
      var maxVisibleSuggestions = this.editor.getOption(89 /* suggest */)
        .maxVisibleSuggestions;
      height =
        Math.min(suggestionCount, maxVisibleSuggestions) * this.unfocusedHeight;
    }
  ...
  SuggestWidget.prototype.adjustDocsPosition = function () {
    ...
    var cursorCoords = this.editor.getScrolledVisiblePosition(
    ...
    if (
      this.docsPositionPreviousWidgetY &&
    ...
    if (
      ...
      this.listElement.style.marginTop =
        this.details.element.offsetHeight -
  ...
  SuggestWidget.prototype.expandSideOrBelow = function () {
    if (
      !canExpandCompletionItem(this.focusedItem) &&
      this.firstFocusInCurrentList
    ) {
      removeClass(this.element, 'docs-side');
      removeClass(this.element, 'docs-below');
      return;
  ...
  Object.defineProperty(SuggestWidget.prototype, 'maxWidgetHeight', {
    ...
    get: function () {
      return (
        ...
        this.editor.getOption(89 /* suggest */).maxVisibleSuggestions
      );
    },
    enumerable: true,
    configurable: true,
  ...
  Object.defineProperty(SuggestWidget.prototype, 'unfocusedHeight', {
    get: function () {
      var options = this.editor.getOptions();
      return (
        options.get(91 /* suggestLineHeight */) ||
        options.get(34 /* fontInfo */).lineHeight
      );
    },
    enumerable: true,
    configurable: true,
  ...
  SuggestWidget.LOADING_MESSAGE = nls.localize(
    'suggestWidget.loading',
    'Loading...'
  );
  SuggestWidget.NO_SUGGESTIONS_MESSAGE = nls.localize(
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (matchHighlight) {
    collector.addRule(
      '.monaco-editor .suggest-widget .monaco-list .monaco-list-row .monaco-highlighted-label .highlight { color: ' +
        matchHighlight +
        '; }'
    );
  }
  ...
  if (link) {
    collector.addRule(
      '.monaco-editor .suggest-widget a { color: ' + link + '; }'
    );
  }
  var codeBackground = theme.getColor(textCodeBlockBackground);
  if (codeBackground) {
    collector.addRule(
      '.monaco-editor .suggest-widget code { background-color: ' +
        codeBackground +
        '; }'
    );
  }
