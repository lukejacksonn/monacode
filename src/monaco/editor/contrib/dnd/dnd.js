...
var DragAndDropController = /** @class */ (function (_super) {
  ...
  function DragAndDropController(editor) {
    ...
    _this._register(
      _this._editor.onMouseDown(function (e) {
        return _this._onEditorMouseDown(e);
      })
    );
    _this._register(
      _this._editor.onMouseUp(function (e) {
        return _this._onEditorMouseUp(e);
      })
    );
    _this._register(
      _this._editor.onMouseDrag(function (e) {
        return _this._onEditorMouseDrag(e);
      })
    );
    _this._register(
      _this._editor.onMouseDrop(function (e) {
        return _this._onEditorMouseDrop(e);
      })
    );
    _this._register(
      _this._editor.onKeyDown(function (e) {
        return _this.onEditorKeyDown(e);
      })
    );
    _this._register(
      _this._editor.onKeyUp(function (e) {
  ...
  DragAndDropController.prototype._onEditorMouseDrop = function (mouseEvent) {
    if (
      ...
      var newCursorPosition_1 = new Position(
      ...
      if (this._dragSelection === null) {
        ...
        if (mouseEvent.event.shiftKey) {
          ...
          if (primarySelection) {
            var selectionStartLineNumber =
                primarySelection.selectionStartLineNumber,
              selectionStartColumn = primarySelection.selectionStartColumn;
            newSelections = [
              new Selection(
                selectionStartLineNumber,
                selectionStartColumn,
                newCursorPosition_1.lineNumber,
                newCursorPosition_1.column
              ),
            ];
          }
        } else {
          newSelections = (this._editor.getSelections() || []).map(function (
            selection
          ) {
        ...
        !this._dragSelection.containsPosition(newCursorPosition_1) ||
        ((hasTriggerModifier(mouseEvent.event) || this._modifierPressed) &&
          (this._dragSelection.getEndPosition().equals(newCursorPosition_1) ||
            this._dragSelection.getStartPosition().equals(newCursorPosition_1))) // we allow users to paste content beside the selection
      ) {
  ...
  DragAndDropController.prototype.showAt = function (position) {
    ...
    this._dndDecorationIds = this._editor.deltaDecorations(
  ...
  DragAndDropController.TRIGGER_KEY_VALUE = isMacintosh
    ? 6 /* Alt */
    : 5 /* Ctrl */;
