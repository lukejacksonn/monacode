...
} from '../../../platform/browser/contextScopedHistoryWidget.js';
var NLS_FIND_INPUT_LABEL = nls.localize('label.find', 'Find');
var NLS_FIND_INPUT_PLACEHOLDER = nls.localize('placeholder.find', 'Find');
var NLS_PREVIOUS_MATCH_BTN_LABEL = nls.localize(
  'label.previousMatchButton',
  'Previous match'
);
var NLS_NEXT_MATCH_BTN_LABEL = nls.localize(
  'label.nextMatchButton',
  'Next match'
);
var NLS_TOGGLE_SELECTION_FIND_TITLE = nls.localize(
...
var NLS_REPLACE_INPUT_PLACEHOLDER = nls.localize(
  'placeholder.replace',
  'Replace'
);
var NLS_REPLACE_BTN_LABEL = nls.localize('label.replaceButton', 'Replace');
var NLS_REPLACE_ALL_BTN_LABEL = nls.localize(
  'label.replaceAllButton',
  'Replace All'
);
var NLS_TOGGLE_REPLACE_MODE_BTN_LABEL = nls.localize(
  'label.toggleReplaceButton',
  'Toggle Replace mode'
);
var NLS_MATCHES_COUNT_LIMIT_TITLE = nls.localize(
...
var ctrlEnterReplaceAllWarningPromptedKey =
...
var FindWidget = /** @class */ (function (_super) {
  ...
  function FindWidget(
    ...
    _this._ctrlEnterReplaceAllWarningPrompted = !!storageService.getBoolean(
    ...
    _this._register(
      _this._state.onFindReplaceStateChange(function (e) {
    ...
    _this._register(
      _this._codeEditor.onDidChangeConfiguration(function (e) {
        ...
        if (e.hasChanged(28 /* find */)) {
          var addExtraSpaceOnTop = _this._codeEditor.getOption(28 /* find */)
            .addExtraSpaceOnTop;
    ...
    _this.updateAccessibilitySupport();
    _this._register(
      _this._codeEditor.onDidChangeCursorSelection(function () {
        if (_this._isVisible) {
    ...
    _this._register(
      _this._codeEditor.onDidFocusEditorWidget(function () {
        if (_this._isVisible) {
          var globalBufferTerm = _this._controller.getGlobalBufferTerm();
          if (
            globalBufferTerm &&
            globalBufferTerm !== _this._state.searchString
          ) {
    ...
    _this._findInputFocused = CONTEXT_FIND_INPUT_FOCUSED.bindTo(
      contextKeyService
    );
    _this._findFocusTracker = _this._register(
      dom.trackFocus(_this._findInput.inputBox.inputElement)
    );
    _this._register(
      _this._findFocusTracker.onDidFocus(function () {
        _this._findInputFocused.set(true);
        _this._updateSearchScope();
      })
    );
    _this._register(
      _this._findFocusTracker.onDidBlur(function () {
        _this._findInputFocused.set(false);
      })
    );
    _this._replaceInputFocused = CONTEXT_REPLACE_INPUT_FOCUSED.bindTo(
      contextKeyService
    );
    _this._replaceFocusTracker = _this._register(
      dom.trackFocus(_this._replaceInput.inputBox.inputElement)
    );
    _this._register(
      _this._replaceFocusTracker.onDidFocus(function () {
        _this._replaceInputFocused.set(true);
        _this._updateSearchScope();
      })
    );
    _this._register(
      _this._replaceFocusTracker.onDidBlur(function () {
    ...
    _this._register(
      _this._codeEditor.onDidChangeModel(function () {
        if (!_this._isVisible) {
          return;
        }
    ...
    _this._register(
      _this._codeEditor.onDidScrollChange(function (e) {
  ...
  FindWidget.prototype._onStateChanged = function (e) {
    ...
    if (e.isReplaceRevealed) {
      if (this._state.isReplaceRevealed) {
        if (
          !this._codeEditor.getOption(68 /* readOnly */) &&
  ...
  // ----- actions
  FindWidget.prototype._getAriaLabel = function (
    label,
    currentMatch,
    searchString
  ) {
  ...
  FindWidget.prototype._updateButtons = function () {
    ...
    this._prevBtn.setEnabled(
      this._isVisible && findInputIsNonEmpty && matchesCount
    );
    ...
    this._toggleReplaceBtn.toggleClass(
  ...
  FindWidget.prototype._reveal = function () {
    ...
    if (!this._isVisible) {
      ...
      if (
        this._codeEditor.getOption(28 /* find */)
        ...
        if (domNode) {
          ...
          var startCoords = this._codeEditor.getScrolledVisiblePosition(
          ...
          if (this._viewZone && startTop < this._viewZone.heightInPx) {
            ...
            var endCoords = this._codeEditor.getScrolledVisiblePosition(
  ...
  FindWidget.prototype._layoutViewZone = function () {
    var _this = this;
    var addExtraSpaceOnTop = this._codeEditor.getOption(28 /* find */)
      .addExtraSpaceOnTop;
    ...
    this._codeEditor.changeViewZones(function (accessor) {
      ...
      _this._codeEditor.setScrollTop(
        _this._codeEditor.getScrollTop() + viewZone.heightInPx
      );
    });
  };
  FindWidget.prototype._showViewZone = function (adjustScroll) {
    ...
    this._codeEditor.changeViewZones(function (accessor) {
      if (_this._viewZoneId !== undefined) {
        ...
        if (adjustScroll) {
          _this._codeEditor.setScrollTop(
            _this._codeEditor.getScrollTop() + scrollAdjustment
          );
        }
        return;
      } else {
  ...
  FindWidget.prototype._removeViewZone = function () {
    ...
    this._codeEditor.changeViewZones(function (accessor) {
      if (_this._viewZoneId !== undefined) {
        ...
        if (_this._viewZone) {
          _this._codeEditor.setScrollTop(
            _this._codeEditor.getScrollTop() - _this._viewZone.heightInPx
          );
  ...
  FindWidget.prototype._applyTheme = function (theme) {
    var inputStyles = {
      ...
      inputValidationInfoForeground: theme.getColor(
        inputValidationInfoForeground
      ),
      ...
      inputValidationWarningForeground: theme.getColor(
        inputValidationWarningForeground
      ),
      ...
      inputValidationErrorForeground: theme.getColor(
        inputValidationErrorForeground
      ),
  ...
  FindWidget.prototype._tryUpdateWidgetWidth = function () {
    ...
    if (this._resized) {
      ...
      if (widgetWidth > FIND_WIDGET_INITIAL_WIDTH) {
        ...
        this._domNode.style.maxWidth =
          editorWidth - 28 - minimapWidth - 15 + 'px';
        this._replaceInput.width = dom.getTotalWidth(this._findInput.domNode);
        return;
      }
    }
    ...
    if (
      FIND_WIDGET_INITIAL_WIDTH + 28 + minimapWidth - MAX_MATCHES_COUNT_WIDTH >=
      editorWidth
    ) {
  ...
  FindWidget.prototype._buildDomNode = function () {
    ...
    this._findInput.setWholeWords(!!this._state.wholeWord);
    this._register(
      this._findInput.onKeyDown(function (e) {
        return _this._onFindInputKeyDown(e);
      })
    );
    this._register(
      this._findInput.inputBox.onDidChange(function () {
    ...
    this._register(
      this._findInput.onDidOptionChange(function () {
        _this._state.change(
          {
    ...
    this._register(
      this._findInput.onCaseSensitiveKeyDown(function (e) {
        if (e.equals(1024 /* Shift */ | 2 /* Tab */)) {
    ...
    this._register(
      this._findInput.onRegexKeyDown(function (e) {
        if (e.equals(2 /* Tab */)) {
          if (_this._isReplaceVisible) {
    ...
    this._register(
      this._findInput.inputBox.onDidHeightChange(function (e) {
    ...
    if (platform.isLinux) {
      this._register(
        this._findInput.onMouseDown(function (e) {
    ...
    this._register(
      this._toggleSelectionFind.onChange(function () {
    ...
    this._register(
      this._replaceInput.onKeyDown(function (e) {
        return _this._onReplaceInputKeyDown(e);
      })
    );
    this._register(
      this._replaceInput.inputBox.onDidChange(function () {
        _this._state.change(
    ...
    this._register(
      this._replaceInput.inputBox.onDidHeightChange(function (e) {
    ...
    this._register(
      this._replaceInput.onDidOptionChange(function () {
        _this._state.change(
          {
    ...
    this._register(
      this._replaceInput.onPreserveCaseKeyDown(function (e) {
        if (e.equals(2 /* Tab */)) {
    ...
    this._toggleReplaceBtn = this._register(
      new SimpleButton({
        ...
        onTrigger: function () {
          ...
          if (_this._isReplaceVisible) {
            _this._replaceInput.width = dom.getTotalWidth(
              _this._findInput.domNode
            );
    ...
    this._register(
      this._resizeSash.onDidStart(function () {
        originalWidth = dom.getTotalWidth(_this._domNode);
      })
    );
    this._register(
      this._resizeSash.onDidChange(function (evt) {
        _this._resized = true;
        ...
        var maxWidth =
          parseFloat(dom.getComputedStyle(_this._domNode).maxWidth) || 0;
        if (width > maxWidth) {
          return;
        }
    ...
    this._register(
      this._resizeSash.onDidReset(function () {
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (widgetShadowColor) {
    collector.addRule(
      '.monaco-editor .find-widget { box-shadow: 0 2px 8px ' +
        widgetShadowColor +
        '; }'
    );
  }
  ...
  if (findMatchBorder) {
    collector.addRule(
      '.monaco-editor .currentFindMatch { border: 2px solid ' +
        findMatchBorder +
  ...
  if (foreground) {
    collector.addRule(
      '.monaco-editor .find-widget { color: ' + foreground + '; }'
    );
  }
  var error = theme.getColor(errorForeground);
  if (error) {
    collector.addRule(
      '.monaco-editor .find-widget.no-results .matchesCount { color: ' +
        error +
        '; }'
    );
  }
  ...
  if (resizeBorderBackground) {
    collector.addRule(
      '.monaco-editor .find-widget .monaco-sash { background-color: ' +
        resizeBorderBackground +
        '; width: 3px !important; margin-left: -4px;}'
    );
  } else {
  ...
  if (focusOutline) {
    collector.addRule(
      '.monaco-editor .find-widget .monaco-inputbox.synthetic-focus { outline-color: ' +
        focusOutline +
        '; }'
    );
  }
