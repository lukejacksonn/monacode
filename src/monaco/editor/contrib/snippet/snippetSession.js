...
registerThemingParticipant(function (theme, collector) {
  ...
  collector.addRule(
    '.monaco-editor .snippet-placeholder { background-color: ' +
      getColorGraceful(colors.snippetTabstopHighlightBackground) +
      '; outline-color: ' +
      getColorGraceful(colors.snippetTabstopHighlightBorder) +
      '; }'
  );
  collector.addRule(
    '.monaco-editor .finish-snippet-placeholder { background-color: ' +
      getColorGraceful(colors.snippetFinalTabstopHighlightBackground) +
      '; outline-color: ' +
      getColorGraceful(colors.snippetFinalTabstopHighlightBorder) +
      '; }'
  );
});
var OneSnippet = /** @class */ (function () {
  ...
  OneSnippet.prototype._initDecorations = function () {
    ...
    this._editor.changeDecorations(function (accessor) {
      ...
      for (var _i = 0, _a = _this._snippet.placeholders; _i < _a.length; _i++) {
        ...
        var range = Range.fromPositions(
          model.getPositionAt(_this._offset + placeholderOffset),
          model.getPositionAt(
            _this._offset + placeholderOffset + placeholderLen
          )
        );
        var options = placeholder.isFinalTabstop
  ...
  OneSnippet.prototype.move = function (fwd) {
    ...
    if (this._placeholderGroupsIdx >= 0) {
      var operations = [];
      for (
        var _i = 0, _a = this._placeholderGroups[this._placeholderGroupsIdx];
        _i < _a.length;
        _i++
      ) {
        var placeholder = _a[_i];
    ...
    var newSelections = this._editor
      .getModel()
      .changeDecorations(function (accessor) {
        ...
        for (
          var _i = 0,
            _a = _this._placeholderGroups[_this._placeholderGroupsIdx];
          _i < _a.length;
          _i++
        ) {
          var placeholder = _a[_i];
          ...
          couldSkipThisPlaceholder =
            couldSkipThisPlaceholder &&
            _this._hasPlaceholderBeenCollapsed(placeholder);
          ...
          for (
            var _b = 0, _c = _this._snippet.enclosingPlaceholders(placeholder);
            _b < _c.length;
            _b++
          ) {
  ...
  OneSnippet.prototype.computePossibleSelections = function () {
    ...
    for (var _i = 0, _a = this._placeholderGroups; _i < _a.length; _i++) {
      ...
      var ranges = void 0;
      for (
        var _b = 0, placeholdersWithEqualIndex_1 = placeholdersWithEqualIndex;
        _b < placeholdersWithEqualIndex_1.length;
        _b++
      ) {
  ...
  OneSnippet.prototype.merge = function (others) {
    ...
    this._editor.changeDecorations(function (accessor) {
      ...
      for (
        var _i = 0, _a = _this._placeholderGroups[_this._placeholderGroupsIdx];
        _i < _a.length;
        _i++
      ) {
        var placeholder = _a[_i];
        ...
        for (
          var _b = 0, _c = nested._snippet.placeholderInfo.all;
          _b < _c.length;
          _b++
        ) {
          ...
          if (nestedPlaceholder.isFinalTabstop) {
            ...
            nestedPlaceholder.index =
              placeholder.index + nestedPlaceholder.index / _this._nestingLevel;
          }
        }
        ...
        for (
          var _d = 0, _e = nested._snippet.placeholders;
          _d < _e.length;
          _d++
        ) {
          ...
          var handle = accessor.addDecoration(
            range,
            OneSnippet._decor.inactive
          );
      ...
      _this._placeholderGroups = groupBy(
        _this._snippet.placeholders,
        Placeholder.compareByIndex
      );
    });
  };
...
var SnippetSession = /** @class */ (function () {
  ...
  SnippetSession.adjustWhitespace = function (
    model,
    position,
    snippet,
    adjustIndentation,
    ...
    var lineLeadingWhitespace = getLeadingWhitespace(
      line,
      0,
      position.column - 1
    );
        ...
        if (adjustIndentation) {
          for (var i = 1; i < lines.length; i++) {
            ...
            lines[i] =
              ...
              ) + lines[i].substr(templateLeadingWhitespace.length);
          }
        }
        if (adjustNewlines) {
  ...
  SnippetSession.adjustSelection = function (
    model,
    selection,
    overwriteBefore,
    overwriteAfter
  ...
  SnippetSession.createEditsAndSnippets = function (
    editor,
    template,
    overwriteBefore,
    overwriteAfter,
    ...
    var modelBasedVariableResolver = editor.invokeWithinContext(function (
      accessor
    ) {
      return new ModelBasedVariableResolver(
        accessor.get(ILabelService, optional),
        model
      );
    });
    ...
    var readClipboardText = function () {
      return (
        clipboardText || (clipboardService && clipboardService.readTextSync())
      );
    };
    var delta = 0;
    ...
    var firstBeforeText = model.getValueInRange(
    ...
    var firstAfterText = model.getValueInRange(
      SnippetSession.adjustSelection(
        model,
        editor.getSelection(),
        0,
    ...
    var firstLineFirstNonWhitespace = model.getLineFirstNonWhitespaceColumn(
    ...
    // primary selection
    var indexedSelections = editor
      .getSelections()
      .map(function (selection, idx) {
        return { selection: selection, idx: idx };
      })
      .sort(function (a, b) {
        return Range.compareRangesUsingStarts(a.selection, b.selection);
      });
    for (
      var _i = 0, indexedSelections_1 = indexedSelections;
      _i < indexedSelections_1.length;
      _i++
    ) {
      ...
      var extensionAfter = SnippetSession.adjustSelection(
        model,
        selection,
        0,
        overwriteAfter
      ...
      var snippet = new SnippetParser().parse(
        template,
        true,
        enforceFinalTabstop
      );
      ...
      snippet.resolveVariables(
        new CompositeSnippetVariableResolver([
          ...
          new ClipboardBasedVariableResolver(
            ...
            editor.getOption(60 /* multiCursorPaste */) === 'spread'
          ),
      ...
      delta +=
        snippet.toString().length -
        model.getValueLengthInRange(snippetSelection);
  ...
  SnippetSession.prototype.insert = function () {
    ...
    var _a = SnippetSession.createEditsAndSnippets(
        this._editor,
        this._template,
