...
var ParameterHintsWidget = /** @class */ (function (_super) {
  ...
  function ParameterHintsWidget(
    ...
    _this.markdownRenderer = _this._register(
    ...
    _this.keyMultipleSignatures = Context.MultipleSignatures.bindTo(
      contextKeyService
    );
    _this._register(
      _this.model.onChangedHints(function (newParameterHints) {
  ...
  ParameterHintsWidget.prototype.createParamaterHintDOMNodes = function () {
    ...
    var previous = dom.append(
      controls,
      $('.button.codicon.codicon-chevron-up')
    );
    ...
    this._register(
      this.editor.onDidLayoutChange(function (e) {
  ...
  ParameterHintsWidget.prototype.render = function (hints) {
    ...
    if (activeParameter && activeParameter.documentation) {
      ...
      if (typeof activeParameter.documentation === 'string') {
        ...
        var renderedContents = this.markdownRenderer.render(
    ...
    if (activeParameter) {
      var labelToAnnounce = this.getParameterLabel(
        signature,
        hints.activeParameter
      );
  ...
  ParameterHintsWidget.prototype.hasDocs = function (
    signature,
    activeParameter
  ) {
    if (
      ...
      activeParameter.documentation.length > 0
    ) {
      return true;
    }
    if (
      ...
      activeParameter.documentation.value.length > 0
    ) {
      return true;
    }
    if (
      typeof signature.documentation === 'string' &&
      signature.documentation.length > 0
    ) {
      return true;
    }
    if (
      typeof signature.documentation === 'object' &&
      signature.documentation.value.length > 0
    ) {
      return true;
    }
    return false;
  };
  ParameterHintsWidget.prototype.renderParameters = function (
    parent,
    signature,
    currentParameter
  ) {
  ...
  ParameterHintsWidget.prototype.getParameterLabel = function (
    signature,
    paramIdx
  ) {
  ...
  ParameterHintsWidget.prototype.getParameterLabelOffsets = function (
    signature,
    paramIdx
  ) {
    var param = signature.parameters[paramIdx];
...
registerThemingParticipant(function (theme, collector) {
  ...
  if (border) {
    ...
    collector.addRule(
      '.monaco-editor .parameter-hints-widget { border: ' +
        borderWidth +
        'px solid ' +
        border +
        '; }'
    ...
    collector.addRule(
      '.monaco-editor .parameter-hints-widget.multiple .body { border-left: 1px solid ' +
        border.transparent(0.5) +
        '; }'
    );
    collector.addRule(
      '.monaco-editor .parameter-hints-widget .signature.has-docs { border-bottom: 1px solid ' +
        border.transparent(0.5) +
        '; }'
    );
  }
  ...
  if (background) {
    collector.addRule(
      '.monaco-editor .parameter-hints-widget { background-color: ' +
        background +
        '; }'
    );
  }
  ...
  if (link) {
    collector.addRule(
      '.monaco-editor .parameter-hints-widget a { color: ' + link + '; }'
    );
  }
  var foreground = theme.getColor(editorHoverForeground);
  if (foreground) {
    collector.addRule(
      '.monaco-editor .parameter-hints-widget { color: ' + foreground + '; }'
    );
  }
  var codeBackground = theme.getColor(textCodeBlockBackground);
  if (codeBackground) {
    collector.addRule(
      '.monaco-editor .parameter-hints-widget code { background-color: ' +
        codeBackground +
        '; }'
    );
  }
