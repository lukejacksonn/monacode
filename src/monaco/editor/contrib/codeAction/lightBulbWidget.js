...
var LightBulbWidget = /** @class */ (function (_super) {
  ...
  function LightBulbWidget(
    _editor,
    _quickFixActionId,
    _preferredFixActionId,
    _keybindingService
  ) {
    ...
    _this._register(
      _this._editor.onDidChangeModelContent(function (_) {
        // cancel when the line in question has been removed
        var editorModel = _this._editor.getModel();
        if (
          _this.state.type !== 1 /* Showing */ ||
          !editorModel ||
          _this.state.editorPosition.lineNumber >= editorModel.getLineCount()
        ) {
          _this.hide();
        }
      })
    ...
    _this._register(
      dom.addStandardDisposableGenericMouseDownListner(
        _this._domNode,
        function (e) {
          ...
          if (
            _this.state.widgetPosition.position !== null &&
            _this.state.widgetPosition.position.lineNumber <
              _this.state.editorPosition.lineNumber
          ) {
            pad += lineHeight;
          }
          _this._onClick.fire({
    ...
    _this._register(
      dom.addDisposableListener(_this._domNode, 'mouseenter', function (e) {
        if ((e.buttons & 1) !== 1) {
          return;
        }
    ...
    _this._register(
      _this._editor.onDidChangeConfiguration(function (e) {
        // hide when told to do so
        if (
          e.hasChanged(47 /* lightbulb */) &&
          !_this._editor.getOption(47 /* lightbulb */).enabled
        ) {
          _this.hide();
        }
      })
    ...
    _this._register(
      _this._keybindingService.onDidUpdateKeybindings(
        _this._updateLightBulbTitle,
        _this
      )
    );
  ...
  LightBulbWidget.prototype.getPosition = function () {
    return this._state.type === 1 /* Showing */
  ...
  LightBulbWidget.prototype.update = function (actions, trigger, atPosition) {
    ...
    var isFolded = function (lineNumber) {
      return (
        lineNumber > 2 &&
        _this._editor.getTopForLineNumber(lineNumber) ===
          _this._editor.getTopForLineNumber(lineNumber - 1)
      );
    };
  ...
  LightBulbWidget.prototype._updateLightBulbTitle = function () {
    if (this.state.type === 1 /* Showing */ && this.state.actions.hasAutoFix) {
      var preferredKb = this._keybindingService.lookupKeybinding(
        this._preferredFixActionId
      );
      if (preferredKb) {
        this.title = nls.localize(
          'prefferedQuickFixWithKb',
          'Show Fixes. Preferred Fix Available ({0})',
          preferredKb.getLabel()
        );
        return;
      }
    }
...
registerThemingParticipant(function (theme, collector) {
  // Lightbulb Icon
  var editorLightBulbForegroundColor = theme.getColor(
    editorLightBulbForeground
  );
  if (editorLightBulbForegroundColor) {
    collector.addRule(
      '\n\t\t.monaco-editor .contentWidgets .codicon-lightbulb {\n\t\t\tcolor: ' +
        editorLightBulbForegroundColor +
        ';\n\t\t}'
    );
  }
  // Lightbulb Auto Fix Icon
  var editorLightBulbAutoFixForegroundColor = theme.getColor(
    editorLightBulbAutoFixForeground
  );
  if (editorLightBulbAutoFixForegroundColor) {
    collector.addRule(
      '\n\t\t.monaco-editor .contentWidgets .codicon-lightbulb-autofix {\n\t\t\tcolor: ' +
        editorLightBulbAutoFixForegroundColor +
        ';\n\t\t}'
    );
  }
});
