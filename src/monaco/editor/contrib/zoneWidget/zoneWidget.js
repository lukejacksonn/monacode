...
var Arrow = /** @class */ (function () {
  ...
  Arrow.prototype._updateStyle = function () {
    ...
    dom.createCSSRule(
      ...
      'border-style: solid; border-color: transparent; border-bottom-color: ' +
        this._color +
        '; border-width: ' +
        this._height +
        'px; bottom: -' +
...
var ZoneWidget = /** @class */ (function () {
  function ZoneWidget(editor, options) {
    ...
    this._disposables.add(
      this.editor.onDidLayoutChange(function (info) {
  ...
  ZoneWidget.prototype.show = function (rangeOrPos, heightInLines) {
    ...
    this._positionMarkerId = this.editor.deltaDecorations(
      this._positionMarkerId,
      [{ range: range, options: ModelDecorationOptions.EMPTY }]
    );
  };
  ...
  ZoneWidget.prototype._showImpl = function (where, heightInLines) {
    ...
    this.editor.changeViewZones(function (accessor) {
      ...
      _this._viewZone = new ViewZoneDelegate(
        viewZoneDomNode,
        position.lineNumber,
        position.column,
        heightInLines,
      ...
      _this._overlayWidget = new OverlayWidgetDelegate(
        WIDGET_ID + _this._viewZone.id,
        _this.domNode
      );
      _this.editor.addOverlayWidget(_this._overlayWidget);
    });
    if (this.container && this.options.showFrame) {
      var width_1 = this.options.frameWidth
  ...
  ZoneWidget.prototype._initSash = function () {
    ...
    this._resizeSash = this._disposables.add(
      new Sash(this.domNode, this, { orientation: 1 /* HORIZONTAL */ })
    );
    ...
    var data;
    this._disposables.add(
      this._resizeSash.onDidStart(function (e) {
        if (_this._viewZone) {
          data = {
            startY: e.startY,
    ...
    this._disposables.add(
      this._resizeSash.onDidEnd(function () {
        data = undefined;
      })
    );
    this._disposables.add(
      this._resizeSash.onDidChange(function (evt) {
        if (data) {
          var lineDelta =
            ...
            _this.editor.getOption(49 /* lineHeight */);
          var roundedLineDelta =
