...
var SplitView = /** @class */ (function (_super) {
  ...
  function SplitView(container, options) {
    ...
    _this.orientation = types.isUndefined(options.orientation)
      ? 0 /* VERTICAL */
      : options.orientation;
    _this.inverseAltBehavior = !!options.inverseAltBehavior;
    _this.proportionalLayout = types.isUndefined(options.proportionalLayout)
      ? true
    ...
    dom.addClass(
      _this.el,
      _this.orientation === 0 /* VERTICAL */ ? 'vertical' : 'horizontal'
    );
      ...
      options.descriptor.views.forEach(function (viewDescriptor, index) {
        var sizing =
          types.isUndefined(viewDescriptor.visible) || viewDescriptor.visible
            ? viewDescriptor.size
            : { type: 'invisible', cachedVisibleSize: viewDescriptor.size };
  ...
  SplitView.prototype.resizeView = function (index, size) {
    ...
    var lowPriorityIndexes = __spreadArrays(
  ...
  SplitView.prototype.distributeViewSizes = function () {
    ...
    for (
      var _b = 0, flexibleViewItems_1 = flexibleViewItems;
      _b < flexibleViewItems_1.length;
      _b++
    ) {
    ...
    if (this.viewItems.length > 1) {
      ...
      var onDidResetDisposable = sash_1.onDidReset(function () {
        ...
        if (
          ...
          !_this.viewItems[snapBeforeIndex].visible
        ) {
          return;
        }
        if (
          typeof snapAfterIndex === 'number' &&
          !_this.viewItems[snapAfterIndex].visible
        ) {
          return;
        }
        _this._onDidSashReset.fire(index);
      });
      var disposable_1 = combinedDisposable(
        onStartDisposable,
        onChangeDisposable_1,
        onEndDisposable,
  ...
  SplitView.prototype.resize = function (
    index,
    delta,
    sizes,
    lowPriorityIndexes,
    ...
    if (highPriorityIndexes) {
      for (
        var _i = 0, highPriorityIndexes_1 = highPriorityIndexes;
        _i < highPriorityIndexes_1.length;
        _i++
      ) {
    ...
    if (lowPriorityIndexes) {
      for (
        var _a = 0, lowPriorityIndexes_1 = lowPriorityIndexes;
        _a < lowPriorityIndexes_1.length;
        _a++
      ) {
  ...
  SplitView.prototype.distributeEmptySpace = function (lowPriorityIndex) {
    ...
    for (
      var _i = 0, highPriorityIndexes_2 = highPriorityIndexes;
      _i < highPriorityIndexes_2.length;
      _i++
    ) {
    ...
    for (
      var _a = 0, lowPriorityIndexes_2 = lowPriorityIndexes;
      _a < lowPriorityIndexes_2.length;
      _a++
    ) {
