...
var SubmenuAction = /** @class */ (function (_super) {
  ...
  function SubmenuAction(label, entries, cssClass) {
    var _this =
      _super.call(this, !!cssClass ? cssClass : 'submenu', label, '', true) ||
      this;
...
var Menu = /** @class */ (function (_super) {
  ...
  function Menu(container, actions, options) {
        ...
        addDisposableListener(menuElement, EventType.KEY_DOWN, function (e) {
          ...
          if (_this.mnemonics.has(key)) {
            ...
            if (actions_1.length === 1) {
              if (
                actions_1[0] instanceof SubmenuMenuActionViewItem &&
                actions_1[0].container
              ) {
    ...
    _this._register(
      addDisposableListener(_this.domNode, EventType.MOUSE_OUT, function (e) {
    ...
    _this._register(
      addDisposableListener(_this.actionsList, EventType.MOUSE_OVER, function (
        e
      ) {
        var target = e.target;
        if (
          !target ||
          !isAncestor(target, _this.actionsList) ||
          target === _this.actionsList
        ) {
          return;
        }
        while (
          target.parentElement !== _this.actionsList &&
    ...
    // Scroll Logic
    _this.scrollableElement = _this._register(
    ...
    _this._register(
      addDisposableListener(scrollElement, EventType.MOUSE_UP, function (e) {
    ...
    menuElement.style.maxHeight =
      Math.max(
        10,
        window.innerHeight - container.getBoundingClientRect().top - 30
      ) + 'px';
  ...
  Menu.prototype.style = function (style) {
    ...
    if (this.viewItems) {
      this.viewItems.forEach(function (item) {
        if (
          item instanceof BaseMenuActionViewItem ||
          item instanceof MenuSeparatorActionViewItem
        ) {
          item.style(style);
        }
      });
  ...
  Menu.prototype.doGetActionViewItem = function (action, options, parentData) {
    if (action instanceof Separator) {
      ...
      var menuActionViewItem = new SubmenuMenuActionViewItem(
        action,
        action.entries,
        parentData,
        options
      ...
      var menuActionViewItem = new BaseMenuActionViewItem(
        options.context,
        action,
        menuItemOptions
      );
...
var BaseMenuActionViewItem = /** @class */ (function (_super) {
  ...
  function BaseMenuActionViewItem(ctx, action, options) {
    ...
    _this.runOnceToEnableMouseUp = new RunOnceScheduler(function () {
      ...
      _this._register(
        addDisposableListener(_this.element, EventType.MOUSE_UP, function (e) {
  ...
  BaseMenuActionViewItem.prototype.render = function (container) {
    ...
    this.check = append(
      this.item,
      $('span.menu-item-check.codicon.codicon-check')
    );
    ...
    if (this.options.label && this.options.keybinding) {
      append(
        this.item,
        $('span.keybinding')
      ).textContent = this.options.keybinding;
    }
  ...
  BaseMenuActionViewItem.prototype.updatePositionInSet = function (
    pos,
    setSize
  ) {
    if (this.item) {
...
var SubmenuMenuActionViewItem = /** @class */ (function (_super) {
  ...
  function SubmenuMenuActionViewItem(
    action,
    submenuActions,
    parentData,
    submenuOptions
    ...
    _this.hideScheduler = new RunOnceScheduler(function () {
      if (
        _this.element &&
        !isAncestor(document.activeElement, _this.element) &&
        _this.parentData.submenu === _this.mysubmenu
      ) {
  ...
  SubmenuMenuActionViewItem.prototype.render = function (container) {
    ...
    if (this.item) {
      ...
      this.submenuIndicator = append(
        this.item,
        $('span.submenu-indicator.codicon.codicon-chevron-right')
      );
      this.submenuIndicator.setAttribute('aria-hidden', 'true');
    }
    this._register(
      addDisposableListener(this.element, EventType.KEY_UP, function (e) {
        var event = new StandardKeyboardEvent(e);
    ...
    this._register(
      addDisposableListener(this.element, EventType.KEY_DOWN, function (e) {
        var event = new StandardKeyboardEvent(e);
        if (document.activeElement === _this.item) {
          if (
            event.equals(17 /* RightArrow */) ||
            event.equals(3 /* Enter */)
          ) {
            EventHelper.stop(e, true);
          }
        }
    ...
    this._register(
      addDisposableListener(this.element, EventType.MOUSE_OVER, function (e) {
        if (!_this.mouseOver) {
    ...
    this._register(
      addDisposableListener(this.element, EventType.MOUSE_LEAVE, function (e) {
        _this.mouseOver = false;
      })
    );
    this._register(
      addDisposableListener(this.element, EventType.FOCUS_OUT, function (e) {
        if (
          _this.element &&
          !isAncestor(document.activeElement, _this.element)
        ) {
    ...
    this._register(
      this.parentData.parent.onScroll(function () {
        _this.parentData.parent.focus(false);
        _this.cleanupExistingSubmenu(false);
      })
    );
  ...
  SubmenuMenuActionViewItem.prototype.cleanupExistingSubmenu = function (
    force
  ) {
    if (
  ...
  SubmenuMenuActionViewItem.prototype.createSubmenu = function (
    selectFirstItem
  ) {
    var _this = this;
    ...
    if (!this.parentData.submenu) {
      ...
      this.submenuContainer.style.top =
        ...
        this.parentData.parent.scrollOffset -
        paddingTop +
        'px';
      ...
      this.submenuDisposables.add(
        this.parentData.submenu.onDidCancel(function () {
          _this.parentData.parent.focus();
          _this.cleanupExistingSubmenu(true);
        })
      );
  ...
  SubmenuMenuActionViewItem.prototype.updateAriaExpanded = function (value) {
    ...
    if (this.item) {
      (_a = this.item) === null || _a === void 0
        ? void 0
        : _a.setAttribute('aria-expanded', value);
    }
  };
...
var MenuSeparatorActionViewItem = /** @class */ (function (_super) {
  ...
  MenuSeparatorActionViewItem.prototype.style = function (style) {
    if (this.label) {
      this.label.style.borderBottomColor = style.separatorColor
