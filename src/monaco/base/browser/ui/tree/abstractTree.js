...
var TreeElementsDragAndDropData = /** @class */ (function (_super) {
  ...
  function TreeElementsDragAndDropData(data) {
    var _this =
      _super.call(
        this,
        data.elements.map(function (node) {
          return node.element;
        })
      ) || this;
    _this.data = data;
...
var TreeNodeListDragAndDrop = /** @class */ (function () {
  ...
  TreeNodeListDragAndDrop.prototype.getDragLabel = function (
    nodes,
    originalEvent
  ) {
  ...
  TreeNodeListDragAndDrop.prototype.onDragStart = function (
    data,
    originalEvent
  ) {
  ...
  TreeNodeListDragAndDrop.prototype.onDragOver = function (
    data,
    targetNode,
    targetIndex,
    originalEvent,
    ...
    var result = this.dnd.onDragOver(
  ...
  TreeNodeListDragAndDrop.prototype.drop = function (
    data,
    targetNode,
    targetIndex,
    originalEvent
...
function asListOptions(modelProvider, options) {
  return (
    ...
    __assign(__assign({}, options), {
      ...
      dnd:
        options.dnd && new TreeNodeListDragAndDrop(modelProvider, options.dnd),
      multipleSelectionController: options.multipleSelectionController && {
        isSelectionSingleChangeEvent: function (e) {
          return options.multipleSelectionController.isSelectionSingleChangeEvent(
            __assign(__assign({}, e), { element: e.element })
          );
        },
      ...
      accessibilityProvider:
        ...
        __assign(__assign({}, options.accessibilityProvider), {
      ...
      keyboardNavigationLabelProvider:
        ...
        __assign(__assign({}, options.keyboardNavigationLabelProvider), {
      ...
      ariaProvider: {
        ...
        isChecked:
          options.ariaProvider && options.ariaProvider.isChecked
            ? function (node) {
                return options.ariaProvider.isChecked(node.element);
              }
            : undefined,
        getRole:
          options.ariaProvider && options.ariaProvider.getRole
            ? function (node) {
...
var TreeRenderer = /** @class */ (function () {
  ...
  TreeRenderer.prototype.renderElement = function (
    node,
    index,
    templateData,
    height
  ...
  TreeRenderer.prototype.disposeElement = function (
    node,
    index,
    templateData,
    height
...
var TypeFilterController = /** @class */ (function () {
  function TypeFilterController(
    ...
    domEvent(this.domNode, 'dragstart')(
      this.onDragStart,
      this,
      this.disposables
    );
    this.messageDomNode = append(
      view.getHTMLElement(),
    ...
    this.filterOnTypeDomNode = append(
      controls,
      $('input.filter.codicon.codicon-list-selection')
    );
    this.filterOnTypeDomNode.type = 'checkbox';
    this.filterOnTypeDomNode.checked = this._filterOnType;
    this.filterOnTypeDomNode.tabIndex = -1;
    this.updateFilterOnTypeTitle();
    domEvent(this.filterOnTypeDomNode, 'input')(
      this.onDidChangeFilterOnType,
      this,
      this.disposables
    );
    this.clearDomNode = append(
      controls,
      $('button.clear.codicon.codicon-close')
    );
    this.clearDomNode.tabIndex = -1;
    this.clearDomNode.title = localize('clear', 'Clear');
    this.keyboardNavigationEventFilter =
      tree.options.keyboardNavigationEventFilter;
  ...
  TypeFilterController.prototype.enable = function () {
    ...
    var onKeyDown = Event.chain(domEvent(this.view.getHTMLElement(), 'keydown'))
      .filter(function (e) {
        return (
          !isInputElement(e.target) || e.target === _this.filterOnTypeDomNode
        );
      })
      .filter(function (e) {
      ...
      .filter(function (e) {
        return (
          _this.keyboardNavigationDelegate.mightProducePrintableCharacter(e) ||
          ((_this.pattern.length > 0 || _this.triggered) &&
          (e.keyCode === 9 /* Escape */ || e.keyCode === 1) /* Backspace */ &&
            !e.altKey &&
            !e.ctrlKey &&
            !e.metaKey) ||
          (e.keyCode === 1 /* Backspace */ &&
            (isMacintosh ? e.altKey && !e.metaKey : e.ctrlKey) &&
            !e.shiftKey)
        );
      })
      .forEach(function (e) {
    ...
    Event.chain(Event.any(onKeyDown, onClear)).event(
      this.onEventOrInput,
      this,
    ...
    if (typeof e === 'string') {
      ...
      this.onInput(
        this.pattern.length === 0
          ? ''
          : this.pattern.substr(0, this.pattern.length - 1)
      );
    } else {
  ...
  TypeFilterController.prototype.onDragStart = function () {
    ...
    var onDragEnd = function () {
      ...
      _this.domNode.className =
        'monaco-list-type-filter ' + _this.positionClassName;
  ...
  TypeFilterController.prototype.updateFilterOnTypeTitle = function () {
    if (this.filterOnType) {
      this.filterOnTypeDomNode.title = localize(
  ...
  TypeFilterController.prototype.render = function () {
    ...
    this.labelDomNode.textContent =
      this.pattern.length > 16
        ? '…' + this.pattern.substr(this.pattern.length - 16)
        : this.pattern;
...
var Trait = /** @class */ (function () {
  ...
  Trait.prototype.onDidModelSplice = function (_a) {
    ...
    var deletedNodesVisitor = function (node) {
      return deletedNodesIdSet.add(
        _this.identityProvider.getId(node.element).toString()
      );
    };
    deletedNodes.forEach(function (node) {
...
var TreeNodeList = /** @class */ (function (_super) {
  ...
  function TreeNodeList(
    ...
    var _this =
      _super.call(this, user, container, virtualDelegate, renderers, options) ||
      this;
  ...
  TreeNodeList.prototype.splice = function (start, deleteCount, elements) {
    ...
    if (additionalFocus.length > 0) {
      _super.prototype.setFocus.call(
        this,
        distinctES6(
          __spreadArrays(_super.prototype.getFocus.call(this), additionalFocus)
        )
      );
    }
    if (additionalSelection.length > 0) {
      _super.prototype.setSelection.call(
        this,
        distinctES6(
          __spreadArrays(
            _super.prototype.getSelection.call(this),
            additionalSelection
          )
        )
      );
  ...
  TreeNodeList.prototype.setSelection = function (
    indexes,
    browserEvent,
    fromAPI
  ) {
...
var AbstractTree = /** @class */ (function () {
  function AbstractTree(user, container, delegate, renderers, _options) {
    ...
    onDidChangeActiveNodes.input = Event.chain(
      ...
      .map(function () {
        ...
        for (
          var _b = 0, _c = _this.selection.getNodes();
          _b < _c.length;
          _b++
        ) {
          var node = _c[_b];
    ...
    if (_options.keyboardNavigationLabelProvider) {
      var delegate_1 =
        ...
        DefaultKeyboardNavigationDelegate;
      this.typeFilterController = new TypeFilterController(
        this,
        this.model,
        this.view,
        filter,
  ...
  Object.defineProperty(AbstractTree.prototype, 'openOnSingleClick', {
    get: function () {
      return typeof this._options.openOnSingleClick === 'undefined'
        ? true
        : this._options.openOnSingleClick;
    },
    enumerable: true,
    configurable: true,
  });
  ...
  AbstractTree.prototype.style = function (styles) {
    ...
    if (styles.treeIndentGuidesStroke) {
      content.push(
        '.monaco-list' +
          suffix +
          ':hover .monaco-tl-indent > .indent-guide, .monaco-list' +
          suffix +
          '.always .monaco-tl-indent > .indent-guide  { border-color: ' +
          styles.treeIndentGuidesStroke.transparent(0.4) +
          '; }'
      );
      content.push(
        '.monaco-list' +
          suffix +
          ' .monaco-tl-indent > .indent-guide.active { border-color: ' +
          styles.treeIndentGuidesStroke +
          '; }'
      );
    }
