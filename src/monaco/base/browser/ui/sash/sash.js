...
var Sash = /** @class */ (function (_super) {
  ...
  function Sash(container, layoutProvider, options) {
    ...
    _this.orthogonalStartSashDisposables = _this._register(
      new DisposableStore()
    );
    ...
    _this._register(
      domEvent(_this.el, 'dblclick')(_this.onMouseDoubleClick, _this)
    );
    _this._register(Gesture.addTarget(_this.el));
    _this._register(
      domEvent(_this.el, EventType.Start)(_this.onTouchStart, _this)
    );
    if (isIPad) {
    ...
    set: function (sash) {
      ...
      if (sash) {
        this.orthogonalStartSashDisposables.add(
          sash.onDidEnablementChange(
            this.onOrthogonalStartSashEnablementChange,
            this
          )
        );
    ...
    set: function (sash) {
      ...
      if (sash) {
        this.orthogonalEndSashDisposables.add(
          sash.onDidEnablementChange(
            this.onOrthogonalEndSashEnablementChange,
            this
          )
        );
  ...
  Sash.prototype.onTouchStart = function (event) {
    ...
    listeners.push(
      addDisposableListener(this.el, EventType.Change, function (event) {
    ...
    listeners.push(
      addDisposableListener(this.el, EventType.End, function (event) {
        _this._onDidEnd.fire();
        dispose(listeners);
      })
    );
